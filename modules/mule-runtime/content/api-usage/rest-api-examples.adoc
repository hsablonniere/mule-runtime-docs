...
:page-id: mule-runtime@3.8::api-usage/rest-api-examples
...
* xref:api-usage/rest-api-examples#consuming-a-rest-api-studio[Consuming a REST API]
+
Describes how to write an API that retrieves data from a publicly available REST API
+
* xref:api-usage/rest-api-examples#extracting-and-storing-xml-data-from-a-rest-api-studio[Extracting and Storing XML Data from a REST API]
+
...
The first element, an xref:connectors:http/http-connector#[HTTP listener], listens on localhost port 8081 (the default) for incoming GET requests. Calling the listener from a browser triggers the flow. Use the following syntax for requests:
...
The first element, an xref:connectors:http/http-connector#[HTTP listener], listens on localhost port 8081 (the default) for incoming GET requests. Calling the listener triggers the flow. Requests to the HTTP listener must take the form:
...
The HTTP request connector passes the XML it receives from the API to a byte-array-to-string transformer, which converts the message payload to a string. Next, a xref:routers/splitter-flow-control-reference#[splitter] splits the message into parts. The splitter is configured to split the incoming message using the  `<quote`> element in the XML as delimiter.
...
. Install the correct xref:connectors:database/database-connector#[database driver] for your database in your Studio application.
.  xref:connectors:database/database-connector#adding-the-database-driver-for-generic-db-configuration[Add the Postgres driver to the build path.]
. Start Postgres.
