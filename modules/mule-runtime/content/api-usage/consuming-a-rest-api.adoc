...
:page-id: mule-runtime@3.8::api-usage/consuming-a-rest-api
...
. Click image:add.png[add] next to Connector Configuration to create a new xref:mule-configuration/global-elements#[global element] that encapsulates configuration parameters for the connector.
...
You can use the xref:dataweave/dataweave#[Transform Message component] or a JSON-to-Object transformer to transform this response into another format from which you can extract information, such as route messages.
...
If you work with complex authentication protocols such as OAuth, you can xref:anypoint-connector-devkit@3.8::about-devkit#[build your own Anypoint Connector] to consume the API.

=== HTTPS

If the REST API you are consuming requires incoming requests arrive via HTTPS, you can configure a global HTTPS connector in your Mule application, then reference the connector in your outbound endpoint. In this example, you create a Java keystore file (JKS) and xref:connectors:tls/tls-configuration#[configure TLS].
...
+
You can use xref:mule-configuration/configuring-properties#[property placeholders] for credentials and define the properties in your `mule-app.properties` file.
+
...
If you are calling a REST API with a POST request, you need to obtain the API schema for the POST and match that format in the payload of the Mule message that you send to the API with your request. A good way to do this is to insert a xref:dataweave/dataweave#[Transform Message component] before the HTTP outbound endpoint in your flow, then define the output format in the properties editor.
