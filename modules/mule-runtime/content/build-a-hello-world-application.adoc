...
:page-id: mule-runtime@3.7::build-a-hello-world-application
...
spend much time explaining components. More detailed explanations and concepts exist in the xref:general@master::to-get-anypoint-platform#[Anypoint Fundamentals].
...
This tutorial assumes that you have link:/mule-user-guide/v/3.7/download-and-launch-anypoint-studio[downloaded, installed, and launched Anypoint Studio], and that you've read through the xref:anypoint-studio@5::about-anypoint-studio#[Anypoint Studio Essentials].
...
Anypoint Studio embeds a server that you use for testing deployments as you code. You can deploy your application on the embedded server to see it working and perform any debugging activities before deploying to a production environment, such as an xref:anypoint-platform@latest::runtime-manager:deploying-to-your-own-servers#[on-premises server] or xref:anypoint-platform@latest::cloudhub:cloudhub#[CloudHub].
...
* *NEXT*: Dive into the more extensive *First Day* content starting with xref:mule-concepts#[Mule Concepts].
* Happier using the command line to run applications? Learn more about xref:deploy/starting-and-stopping-mule-esb#[Starting and stopping Mule].
* Skip ahead in the syllabus to the xref:anypoint-studio@5::basic-studio-tutorial#[Basic Studio Tutorial].
