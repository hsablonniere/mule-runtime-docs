...
:page-id: mule-runtime@3.7::shared-resources/sharing-custom-configuration-fragments
...
If this flow-ref is in the same application, no further configuration is needed to call the subflow. However, if this flow-ref is in a different application than the subflow that it calls, you need to first package the shared config file containing the subflow in a JAR file. Then, use Maven's dependency mechanism described in link:/mule-user-guide/v/3.7/sharing-custom-code[Sharing Custom Code] to include it in your application. Finally, import the config file as follows.
