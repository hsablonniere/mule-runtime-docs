...
:page-id: mule-runtime@3.8::shared-resources/domain-project-shared-resources
...
* Adding properties in the configuration files of applications that use shared resources can result in issues, as these properties are shared with other apps in the domain and there may be conflicts. You can instead xref:administer/setting-environment-variables#[set environment variables].
...
Share xref:connectors:tls/tls-configuration#[TLS configurations] allows shared <<HTTP,listener configurations>> to use the same TLS configuration. +
...
* Note that although shared resources is limited to the selected connectors and libraries covered in this document, there are ways to share other configuration fragments in Mule. Refer to xref:shared-resources/sharing-custom-configuration-fragments#[Sharing Custom Configuration Fragments] for details.

== See Also

Access reference documentation for:

* xref:transports/vm-transport-reference#[VM]
* xref:connectors:database/database-connector#[DB]
* xref:transports/jms-transport-reference#[JMS]
* xref:connectors:http/http-connector#[HTTP Connector]
* xref:transports/mule-wmq-transport-reference#[WMQ]
* xref:modules/jboss-transaction-manager-reference#[JBoss]
* xref:administer/setting-environment-variables#[Set Environment Variables]
