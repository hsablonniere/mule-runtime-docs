...
:page-id: mule-runtime@3.7::transactions/using-bitronix-to-manage-transactions
...
This document assumes you are familiar with working with xref:shared-resources/domain-project-shared-resources#[Shared Resources] in Mule.
...
Bitronix integration provides a datasource connection pool for the xref:connectors:database/database-connector#[Database connector], which you must use whenever you want to implement resource pooling in the database connector.
...
xref:shared-resources/domain-project-shared-resources#[define a Bitronix transaction manager as a shared resource] for multiple applications associated with a domain.
