...
:page-id: mule-runtime@3.7::transactions/transaction-management
...
|xref:transactions/single-resource-transactions#[Single Resource Transactions] a|
...
|xref:transactions/multiple-resource-transactions#[Multiple Resource Transactions] a|
...
|xref:transactions/xa-transactions#[XA Transactions] a|
* Receives or sends messages to more than one resource.
...
Use xref:transactions/transactions-configuration-reference#[Transactions Configuration Reference] for quick access to attribute configurations.
...
_Apply a transaction to a flow that involves a single resource. See xref:transactions/single-resource-transactions#[Single Resource Transaction] for details.
|XA Transaction |Apply a transaction to a flow that involves multiple resources: JMS, VM or JDBC. See xref:transactions/xa-transactions#[XA Transaction] for details.
|Multi Transaction |Apply a transaction to a flow that involves multiple resources: JMS or VM. See xref:transactions/multiple-resource-transactions#[Multiple Resource Transaction] for details.
...
* If you apply an XA transaction to a JMS inbound connector in your flow, you have the option of specifying the polling frequency of the queue. Access xref:transactions/xa-transactions#[XA Transactions] for configuration details.
...
Refer to the xref:exceptions/error-handling#[Error Handling] documentation to learn more about Mule's default exception strategy and how to apply exception strategies to flows. Follow the steps below to apply an exception strategy to an individual transaction. 
...
+
. Configure the exception strategy as needed, keeping in mind Mule uses this exception strategy to handle any exceptions thrown while processing the transaction. Reference the xref:exceptions/error-handling#[Error Handling] documentation for exception strategy configuration details.
...
* Read more about xref:transactions/single-resource-transactions#[Single Resource Transactions], xref:transactions/multiple-resource-transactions#[Multiple Resource Transactions], and xref:transactions/xa-transactions#[XA Transactions]. 
* Read more about xref:exceptions/error-handling#[Exception Strategies].
...
* Use the xref:transactions/transactions-configuration-reference#[Transactions Configuration Reference] for quick reference to transactional attributes.