...
:page-id: mule-runtime@3.8::transactions/reliability-patterns
...
❶ The flow specifies a synchronous processing strategy (`processingStrategy="synchronous"`). In a synchronous processing strategy, the entire flow is processed in the receiver thread. This ensures that the transfer to the VM endpoint happens in the same thread. See xref:flows/flow-processing-strategies#[Flow Processing Strategies] for further details about the synchronous processing strategy. +
