...
:page-id: mule-runtime@3.8::dataweave/dataweave-quickstart
...
[TIP]
For advanced examples that involve writing DataWeave code, see xref:dataweave/dataweave-examples#[DataWeave Examples].
...
. Drag an xref:connectors:http/http-connector#[HTTP Connector] from the palette onto your empty canvas, followed by a *Transform Message* component, then another HTTP Connector.
+
...
See how the expression that evaluates `size` has changed--it uses the xref:dataweave/dataweave-language-introduction#unless-otherwise[Unless and Otherwise operators] to set the value to `M` _unless_ the buyer's state is Texas, in which case the shirt size, expressed as `size` becomes `XXL`.
...
Click on your Transform Message component. In the xref:anypoint-studio@6::using-dataweave-in-studio#the-dataweave-text-editor[DataWeave text editor] replace the curly brackets '{}' for 'payload'.
...
You can easily change the xref:dataweave/dataweave-language-introduction#output-directive[output directive] from the default `application/java` to `text/csv` or to any other of the xref:dataweave/dataweave-formats#[supported formats] and see the results of that transformation.
...
Change the xref:dataweave/dataweave-language-introduction#output-directive[output directive] to `application/json` so that the final result is identical to the sample data in the input. Later on you can transform this in more interesting ways.
...
As the input is a collection of elements, each describing a product in inventory, this code uses the xref:dataweave/dataweave-operators#map[map operator] to treat each of these instances separately. What's enclosed in curly brackets right after the map operator is executed once for each element in the collection. Within this operation, the code refers to the currently selected instance via the `$` selector. The code in this example merely takes each item of the input and maps it to an identical field, with the exception of the *size* field, on which it applies the xref:dataweave/dataweave-operators#upper[upper operator] to change it to upper case.
...
The result from the previous step includes a products for which there is zero stock. We can easily filter those cases out using the xref:dataweave/dataweave-operators#filter[Filter operator]. Modify your DataWeave code to include `filter $.count > 0` in the end. Your code should look like this:
...
[TIP]
See the xref:dataweave/dataweave-language-introduction#precedence-table[Precedence Table] to have a clear idea about when each expression is compiled.
...
* More advanced examples in xref:dataweave/dataweave-examples#[DataWeave Examples]
...
* Migrate your old DataMapper transforms automatically by using the xref:dataweave/dataweave-migrator#[DataWeave Migrator Tool]
