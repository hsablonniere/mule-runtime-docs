...
:page-id: mule-runtime@3.8::dataweave/dataweave-language-introduction
:keywords: studio, anypoint, esb, transform, transformer, format, aggregate, rename, split, filter convert, xml, json, csv, pojo, java object, metadata, dataweave, data weave, datamapper, dwl, dfl, dw, output structure, input structure, map, mapping

image:dw-logo.png[dw]

The xref:dataweave/dataweave#[DataWeave] Language is a powerful template engine that allows you to transform data to and from any kind of format (XML, CSV, JSON, Pojos, Maps, etc). In Anypoint Studio, this language can be used in:

* The *Transform Message Component*, described in xref:anypoint-studio@6::using-dataweave-in-studio#[Using DataWeave in Studio]
* Any Mule component that accepts Mule Expression Language, through xref:mel/mel-dataweave-functions#[MEL DataWeave Functions]
...
* For a listing and details about all of the types you can use, see xref:dataweave/dataweave-types#[DataWeave Types]
...
* xref:dataweave/dataweave-formats#java[`application/java`]
* xref:dataweave/dataweave-formats#csv[`application/csv`]
* xref:dataweave/dataweave-formats#csv[`text/csv`]
* xref:dataweave/dataweave-formats#json[`application/json`]
* xref:dataweave/dataweave-formats#json[`text/json`]
* xref:dataweave/dataweave-formats#xml[`application/xml`]
* xref:dataweave/dataweave-formats#xml[`text/xml`]
* xref:dataweave/dataweave-formats#flat-file[`text/plain`]
* `application/dw`
...
You can define a xref:dataweave/dataweave-types#functions-and-lambdas[function] in the header, you can then call it in any part of the DataWeave body, including arguments.
...
See xref:dataweave/dataweave-types#functions-and-lambdas[Functions and Lambdas] for more on this.
...
* xref:dataweave/dataweave-types#string[*String*] : Double quoted ("Hello") or Single quoted ('Hello')
* xref:dataweave/dataweave-types#boolean[*Boolean*] : Literals true or false
* xref:dataweave/dataweave-types#number[*Number*] : Decimal and Integer values are supported (ex: 2.0)
* xref:dataweave/dataweave-types#dates[*Date*] : IS0-8601 enclosed by "|" (ex:|2003-10-01T23:57:59Z|)
* xref:dataweave/dataweave-types#regular-expressions[*Regex*] : Regex expression enclosed by "/" (ex:/(\d+)-(\d+)/)
...
See xref:dataweave/dataweave-types#array[DataWeave types] for more details on arrays.
...
See xref:dataweave/dataweave-types#object[DataWeave types] for more details on objects.
...
Operators can be classified based on their link:https://en.wikipedia.org/wiki/Arity[arity] as Unary, Binary or Ternary. See xref:dataweave/dataweave-operators#[DataWeave Operators] for a full reference.
...
A selector allows for the navigation and querying the multiple levels of a data-structure to reference a certain value or set of values. See xref:dataweave/dataweave-selectors#[DataWeave Selectors] for a full reference.
...
[TIP]
For simpler use cases where all you need is a boolean result based on if a value matches or not, see the xref:dataweave/dataweave-operators#matches[Matches Operator].
...
In this example, the first field evaluates the type of 'payload.a' and returns a different string with the type name depending on what type it matches with (this could be easier done through the xref:dataweave/dataweave-operators#type-of[Type Of operator]), the second field returns an object with the same type name as a string and a reference to the evaluated value.
...
In this example, the payload includes two elements in an array, and in both cases the xref:dataweave/dataweave-operators#map[Map operator] to cycle through the array. It then evaluates each element against a regular expression and outputs a different object depending on what kind of match is found.
...
Returns the present moment in xref:dataweave/dataweave-types#dates[(:datetime)] type.
...
[TIP]
See xref:dataweave/dataweave-selectors#[DataWeave Selectors] for a list of possible selectors to use here.


=== Random

Returns a random number of type xref:dataweave/dataweave-types#number[(:number)] between 0 and 1
...
[NOTE]
The `lookup` function does not support calling xref:flows/flows-and-subflows#types-of-flows[subflows].

== Calling Global MEL Functions from DataWeave Code

If you define a global xref:mel/mule-expression-language-mel#[Mule Expression Language] (MEL) function in your Mule project, you can then invoke it anywhere in your DataWeave code, without need for any special syntax.
...
|xref:dataweave/dataweave-operators#AND[AND]                              |Conditional And         | 7
|xref:dataweave/dataweave-operators#OR[OR]                                |Conditional OR          | 8
|xref:dataweave/dataweave-operators#default[Default], <<Pattern Matching>>, xref:dataweave/dataweave-operators#matches[Matches], xref:dataweave/dataweave-operators#map[Map], xref:dataweave/dataweave-operators#map-object[Map Object], xref:dataweave/dataweave-operators#group-by[Group By], xref:dataweave/dataweave-operators#filter[Filter]             |Default Value / Pattern Matching / Binary Operators | 9
...
* For a reference about all of the operators that are available for the `using` operator, see xref:dataweave/dataweave-operators#[DataWeave Operators]
* For a listing and details about all of the types you can use, see xref:dataweave/dataweave-types#[DataWeave Types]
* For details on the different formats you can process with DataWeave and the parameters you can configure for each format, see xref:dataweave/dataweave-formats#[DataWeave Formats]
* For details on how you can select certain components of the incoming message, see xref:dataweave/dataweave-selectors#[DataWeave Selectors]
