...
:page-id: mule-runtime@3.8::dataweave/dataweave-operators
:keywords: studio, anypoint, esb, transform, transformer, format, aggregate, rename, split, filter convert, xml, json, csv, pojo, java object, metadata, dataweave, data weave, datamapper, dwl, dfl, dw, output structure, input structure, map, mapping

image:dw-logo.png[dw]

In xref:dataweave/dataweave#[DataWeave] you can carry out many different operations on the elements of a DataWeave transform. This document serves as a reference for all of the available operators in the DataWeave language. See xref:dataweave/dataweave-types#dataweave-operators-sorted-by-type[all operators sorted by type]

* For an introduction to the essentials of the language, see xref:dataweave/dataweave-language-introduction#[DataWeave Language Intro]
* For information on DataWeave's accepted types, see xref:dataweave/dataweave-types#[DataWeave Types]
* For an index of all available operators, categorized by the types you need to supply as parameters, see xref:dataweave/dataweave-types#operators-sorted-by-type[DataWeave Operators Sorted by Type].
* See xref:dataweave/dataweave-types#functions-and-lambdas[Functions and Lambdas] to learn how to create your own DataWeave functions.

[TIP]
Check the xref:dataweave/dataweave-language-introduction#precedence-table[Precedence Table] to see the order in which DataWeave expressions are compiled.
...
[TIP]
====
If you require to filter by key, you need to use xref:dataweave/dataweave-operators#map-object[mapObject] and *when*. For example, to filter the last example by key:
[source,xml,linenums]
----
%dw 1.0
%output application/xml
...
[TIP]
Check the xref:dataweave/dataweave-language-introduction#precedence-table[Precedence Table] to see what expressions are compiled before or after this one.
...
[NOTE]
Check the xref:dataweave/dataweave-types#type-coercion-table[type coercion table] to see what conversions between what types are allowed in DataWeave.
...
[TIP]
For more advanced use cases where you need to output or conditionally process the matched value, see xref:dataweave/dataweave-language-introduction#pattern-matching[Pattern Matching].
...
There are several operators that deal with xref:dataweave/dataweave-types#dates[date related types], which include date, time, localtime, datetime, localdatetime, period, timezone.
...
* For details on how to create and use your own functions, see xref:dataweave/dataweave-types#functions-and-lambdas[Functions and Lambdas]
