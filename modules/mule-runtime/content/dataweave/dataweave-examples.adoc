...
:page-id: mule-runtime@3.8::dataweave/dataweave-examples
:keywords: studio, anypoint, transform, transformer, format, aggregate, rename, split, filter convert, xml, json, csv, pojo, java object, metadata, dataweave, data weave, datamapper, dwl, dfl, dw, output structure, input structure, map, mapping

== Introduction

Below are a series of examples that demonstrate various data transformation approaches expressed in xref:dataweave/dataweave-language-introduction#[DataWeave]. DataWeave code is typically written inside the *Transform Message* component, which is accessible in Anypoint Studio.

[NOTE]
To step through basic DataWeave use cases and exercises in actual Mule applications, see the xref:dataweave/dataweave-quickstart#[DataWeave Quickstart Guide].
...
[TIP]
The xref:dataweave/dataweave-quickstart#[DataWeave Quickstart Guide] walks you this same transformation, showing you how you can create it automatically via the drag-and-drop UI. It then goes into tweaking the code to make it more interesting.
...
In this example, the input contains sensitive information that should be removed. The transform replicates the inbound structure but uses a simple xref:dataweave/dataweave-operators#remove[`remove`] operator to take away specific key:value pairs.

The example goes through the whole set of elements in the input using the xref:dataweave/dataweave-operators#map[`map`] operator.
...
The xref:dataweave/dataweave-operators#map[`map`] operator carries out the same steps for each element in the input array. Through the use of @, attributes are injected into the XML.

[TIP]
See xref:dataweave/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] on how to select XML attributes in DataWeave.
...
The example goes through the whole set of elements in the input using a xref:dataweave/dataweave-operators#map[`map`] operator. It also uses the xref:dataweave/dataweave-language-introduction#when-otherwise[`when`] operator to only select instances that include the field 'insurance'.
...
The example goes through the whole set of elements in the input using  a xref:dataweave/dataweave-operators#map-object[`mapObject`] operator. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the xref:dataweave/dataweave-language-introduction#when-otherwise[`when`] operator together with an xref:dataweave/dataweave-operators#and[`and`] to pick out the specific keys that need to change, and an https://docs.mulesoft.com/mule-user-guide/v/3.8/dataweave-operators#coerce-to-number[example use] of the `as` operator to coerce its type to string.
...
This example takes an XML input and parses it into a different XML arrangement. After a single `<header>` element is copied, a xref:dataweave/dataweave-operators#map[`map` operation] carries out the same steps for each 'item': several fields are passed on without any changes, then the discount and subtotal fields are calculated with references to constants defined in the xref:dataweave/dataweave-language-introduction#the-dataweave-header[header directives] of the transform. A single set of subtotal, tax and total elements are created by performing a xref:dataweave/dataweave-operators#reduce[*reduce*] operation over all of the items in the "items" array, performing calculations that sometimes involve constants defined in the header. The xref:dataweave/dataweave-operators#as[`as`] operator is also used to coerce to a number and then performs xref:dataweave/dataweave-operators#basic-math-operations[basic math] on these numbers.
...
This example takes in an XML file that is grouped separating two types of elements: teachers and students, the transform uses the xref:dataweave/dataweave-operators#group-by[*Group By* operation] to make it into an XML where the top level grouping is "class". Within each class, the students and teachers that are registered in it are listed.

The example goes through the whole set of elements in the input using both a xref:dataweave/dataweave-operators#map-object[*map object* operation] and a xref:dataweave/dataweave-operators#map[*map* operation]. Note that they differ in that map object processes both key and value of its input, rather than just the keys. It also uses the xref:dataweave/dataweave-operators#filter[*filter* operation] to only show the attendees that have the class subject listed as a hobby. As each student may have multiple hobbies, the xref:dataweave/dataweave-selectors#multi-value-selector[*Multi value* selector] is used to refer to them all.
...
In this example, there are three different input JSON files, these three all arrive in one single Mule message, occupying the payload and two xref:dataweave/dataweave-selectors#flow-variables-from-a-mule-message[flow variables]. The payload contains an array of book objects, one flow variable has a set of currency exchange rates, and the other one a query. The transform filters the first input using the conditions passed in the third input, then performs a xref:anypoint-studio@6::using-dataweave-in-studio#map[`map`] to deal with each remaining object separately. Within this map, it defines two variables: *it* and *props*. Through the use of @, attributes are added into the XML tags. A second map operation inside the first one calculates the price of each book for each of the currencies provided in the second input. Another map operation displays each element in the `author` array as a separate <author></author> tag.


[TIP]
See xref:dataweave/dataweave-selectors#flow-variables-from-a-mule-message[flow variables] for how to access flow variables from the input and xref:dataweave/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
The example defines a xref:dataweave/dataweave-types#functions-and-lambdas[function] in the xref:dataweave/dataweave-language-introduction#dataweave-header[dataweave header] and then calls it on the body. It goes through the whole set of elements in the input using  a xref:dataweave/dataweave-operators#map-object[`mapObject`] operator. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the xref:dataweave/dataweave-language-introduction#when-otherwise[`when` and `otherwise`] operators to pick out only instances where an XML attribute exists, and to do nothing otherwise.
It also uses the xref:dataweave/dataweave-operators#match[`match` ] operator.

See xref:dataweave/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
When producing an output in xref:dataweave/dataweave-formats#xml[XML], DataWeave by default outputs every value as enclosed between an opening and a closing tag:
...
[TIP]
See more XML reader and writer properties on xref:dataweave/dataweave-formats#xml[DataWeave formats].


== Configure CSV Reader

DataWeave supports CSV inputs and outputs, by configuring the reader and writer properties, you can adapt it to different conventions regarding separations, line breaks, etc. Other formats also allow for this kind of configuration. See xref:anypoint-studio@6::using-dataweave-in-studio#reader-configuration[Using DataWeave in Studio] to learn how you can do this easily through the UI, or look at the xref:dataweave/dataweave-xml-reference#reader-configuration[DataWeave XML Reference] for the required XML syntax. You can also see a full reference of the reader properties available with CSV format on xref:dataweave/dataweave-formats#csv[DataWeave formats].

Reader properties are set as part of the XML `dw:transform-message` component, whilst writer properties are specified in the DataWeave code itself, as part of the output directive. You can see a full reference to the writer properties available with CSV format on xref:dataweave/dataweave-formats#writer-properties[DataWeave formats].
...
To do this, you can normalize the input with the xref:dataweave/dataweave-operators#replace[replace] operator so that they all match a single pattern, and then turn the resulting string to a date with xref:dataweave/dataweave-operators#coerce-to-date[coerce to date].

To improve the clarity in your code, you can easily abstract the normalizing of your string into custom a xref:dataweave/dataweave-types#functions-and-lambdas[function], declaring it only once on your DW header, and then using it as many times as necessary by simply referencing this function.
...
You can use these regular expressions in a DW xref:dataweave/dataweave-types#functions-and-lambdas[function] that first evaluates a string to see which regular expression it matches, and then converts it through the the corresponding operation. This function should do the following:

. Take the input string as argument
. Match it with regular expressions in order to determine which format it's in
. Run the xref:dataweave/dataweave-operators#coerce-to-date[coerce to date] operator with the corresponding properties
...
The following example makes all "keys" lower case. The xref:dataweave/dataweave-language-introduction#the-dataweave-header[DataWeave header] defines a xref:dataweave/dataweave-types#functions-and-lambdas[function] that receives another function as an argument and then calls that function in the body, applying itself to the keys and not the values it processes. The function that is sent as an argument is a simple one that uses the xref:dataweave/dataweave-operators#lower[`lower`] operator to set the specified text to lower case.

The example goes through the whole set of elements in the input using a xref:dataweave/dataweave-operators#map-object[`mapObject`] operator. It also uses the xref:dataweave/dataweave-operators#dataweave-language-introduction#when-otherwise[`when` and `otherwise`] operators to pick out only instances _when_ an XML attribute exists, and do nothing _otherwise_.
It also uses the xref:dataweave/dataweave-operators#match[`match` ] operator.
...
See xref:dataweave/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] on how to access XML attributes in DataWeave.
...
* The whole set of elements in the input is mapped using a xref:dataweave/dataweave-operators#map[`map`] operator.
* The example also uses the xref:dataweave/dataweave-language-introduction#default[`default`] operator.


[NOTE]
Note that for this example to work, on the payload input you must set the reader configuration so that the 'nullValueOn' attribute is "empty". See xref:anypoint-studio@6::using-dataweave-in-studio#reader-configuration[reader configuration] for instructions on how to set this.
...
* The whole set of elements in the input is mapped using a xref:dataweave/dataweave-operators#map[`map`] operator.
* This example uses the xref:dataweave/dataweave-operators#reduce[`reduce`] operator.
* This example also uses the xref:dataweave/dataweave-language-introduction#when-otherwise[`when`] operator to conditionally display the last field.
* This example also uses the xref:dataweave/dataweave-operators#split-by[`splitBy`] operator to parse the input.
* Finally this example takes advantage of the ability to define a xref:dataweave/dataweave-types#functions-and-lambdas[`function`] for re-use.
...
* xref:dataweave/dataweave-types#[DataWeave Types]
