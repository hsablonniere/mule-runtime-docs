...
:page-id: mule-runtime@3.7::transformers/native-support-for-json
:keywords: anypoint studio, esb, json, java script object notation, java objects, transformers

JSON is now natively supported in Mule, meaning you can work with JSON documents and bind them automatically to Java objects. Further information is available in the xref:modules/json-module-reference#[JSON Module] configuration reference.
