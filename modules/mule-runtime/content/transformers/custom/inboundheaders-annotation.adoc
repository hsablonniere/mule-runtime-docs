...
:page-id: mule-runtime@3.7::transformers/custom/inboundheaders-annotation
:keywords: annotations, inbound headers, custom java code

This annotation controls how the current message inbound headers are passed into a method. The annotation supports, Map, List, single headers, wildcards and optional entries. It can be used on component entry points and xref:transformers/custom/transformer-annotation#[@Transformer] methods.
