...
:page-id: mule-runtime@3.8::transformers/variable-transformer-reference
...
By contrast, _session_ variables specified using the xref:transformers/session-variable-transformer-reference#[Session Variable Transformer] persist as long as the message continues to be processed within your Mule application, regardless of flow. To learn more about message scopes, refer to xref:mule-concepts#[Mule Concepts].
...
* Read about related transformers, the xref:transformers/session-variable-transformer-reference#[session variable transformer] and the xref:transformers/property-transformer-reference#[properties transformer], which you can use to set properties and variables for different scopes.
