...
:page-id: mule-runtime@3.8::object-scopes/unifying-the-mule-registry

Mule 3.7 and newer versions provides upgrades to the lifecycle and xref:object-scopes/dependency-injection#[dependency injection mechanisms], and unifies the Mule Registry. These changes maintain backwards compatibility.
...
When Mule starts, it searches the classpath for `registry-bootstrap.properties` files, which contain keys and class names for objects to automatically add to the registry. For example, the new xref:connectors:http/http-connector#[HTTP Connector] uses a properties file to register a connection manager. The Batch module uses it to register the batch engine, etc.
