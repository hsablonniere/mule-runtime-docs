...
:page-id: mule-runtime@3.7::components/http-response-builder
...
When the xref:connectors:http/http-listener-connector#[HTTP Connector works as a listener], you can configure a status code and a status reason on the connector itself, separately for both success and failure scenarios. To generate a response from an HTTP template, use the *xref:transformers/parse-template-reference#[Parse Template]* element.

See xref:connectors:http/http-connector#[HTTP Connector] for information about the 3.6 and later operation-based HTTP connector. For information on the deprecated HTTP connector, see link:/mule-user-guide/v/3.7/http-connector-deprecated[HTTP Connector - Deprecated].
====
...
* Read about the xref:connectors:http/http-connector#[HTTP Connector]
* Learn how to create and manage a API with the xref:anypoint-platform@latest::api-manager:about-api-manager#[API Manager]