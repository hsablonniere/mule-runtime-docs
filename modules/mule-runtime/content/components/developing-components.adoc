...
:page-id: mule-runtime@3.8::components/developing-components
...
For details on configuring entry point resolvers, see xref:components/entry-point-resolver-configuration-reference#[Entry Point Resolver Configuration Reference].
...
* Design your component in such a way that interface methods can be mapped to outbound endpoints and then use bindings to map these in configuration. For information on how to configure bindings, see xref:components/configuring-java-components#[Configuring Java Components].
====
