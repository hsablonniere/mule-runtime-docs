...
:page-id: mule-runtime@3.7::components/configuring-java-components
:keywords: connectors, anypoint, studio, java, java components, components

Java is the default component type in Mule. Mule provides two link:http://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/api/component/JavaComponent.html[JavaComponent] implementations:
...
For a real-world example of using `<spring-object/>`, see xref:spring-integration/using-spring-beans-as-flow-components#[Using Spring Beans as Flow Components].
...
You can configure entry point resolvers that determine how your component is invoked when a message is received by the flow. See xref:components/developing-components#[Developing Components] for a more detailed description of their functionality.
...
See xref:components/developing-components#[Developing Components] for more information about lifecycles.
...
Binding can be used on Java components and script components. For more information see xref:components/component-bindings#[Component Bindings].
...
For more information about pooling and reference documentation for pooling configuration options, see xref:administer/tuning-performance#[Tuning Performance].
