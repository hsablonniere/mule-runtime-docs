...
:page-id: mule-runtime@3.7::deploy/deploying-to-multiple-environments
...
[TIP]
====
Should you need to encrypt passwords in your properties file, consider using the xref:security/mule-credentials-vault#[Mule Credentials Vault]. Refer to the documentation for xref:security/anypoint-enterprise-security#[Anypoint Enterprise Security] for more information on securing applications in Mule.
====

== Assumptions

This document assumes that you are familiar with the exercise of building and deploying Mule applications in Mule Studio. Consider reading the xref:general@master::to-get-anypoint-platform#[Anypoint Fundamentals] for more information.
...
[TIP]
====
Learn how to encrypt a properties file using the xref:security/mule-credentials-vault#[Mule Credentials Vault].
====
...
[TIP]
====
Learn more about xref:mule-configuration/configuring-properties#[configuring variable properties].
====

. To configure Studio to use a specific environment when  *deploying on-premise* , *debugging*  or  *using xref:anypoint-studio@5::datasense/datasense#[DataSense]*  to retrieve metadata from a SaaS provider, double-click to open your application's  `mule-project.xml` file, located in the root directory of your project.
+
[WARNING]
====
...
* Learn more about encrypting a properties file using the xref:security/mule-credentials-vault#[Mule Credentials Vault].

* To learn more about deploying to test environments in CloudHub, access the CloudHub Sandbox Environments documentation.

* Learn more about xref:mule-configuration/configuring-properties#[Properties Placeholders] in Mule. 

* Learn how to proceed when deploying multiple applications with xref:shared-resources/domain-project-shared-resources#[Shared Resources]