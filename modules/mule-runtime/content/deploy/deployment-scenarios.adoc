...
:page-id: mule-runtime@3.7::deploy/deployment-scenarios
:keywords: deploy, deploying, cloudhub, on premises, on premise

[IMPORTANT]
This document covers the ways in which Mule runtime can be used to deploy applications in a standalone way. To take full advantage of the available management and monitoring features, you should xref:anypoint-platform@latest::runtime-manager:deploying-to-your-own-servers#[deploy to your runtimes] via the link:anypoint.mulesoft.com[Anypoint Platform]. Read how this and other deployment strategies work xref:anypoint-platform@latest::runtime-manager:deployment-strategies#[here].
...
[TIP]
Learn about other xref:anypoint-platform@latest::runtime-manager:deployment-strategies#[deployment strategies] that leverage link:anypoint.mulesoft.com[Anypoint Platform].
...
You can also run multiple applications side-by-side in a Mule instance using the Mule Deployment Model. This model supports live deployment and hot redeployment of applications.  Further, Mule Standalone fully support for xref:administer/clustering/mule-high-availability-ha-clusters#[Mule High Availability] module and use of the link:/mule-management-console/v/3.7[Mule management console].

For more information on running Mule standalone, see xref:deploy/mule-deployment-model#[Mule Deployment Model].
...
You can start and stop Mule from a Java application or embed it in a Webapp (such as a JSP or servlet). For general instructions, see xref:deploy/embedding-mule-in-a-java-application-or-webapp#[Embedding Mule in a Java Application or Webapp]. Listed below are links to documentation which offer application server-specifc information regarding Mule application deployment.

* xref:deploy/deploying-mule-to-jboss#[Deploying Mule to JBoss]
* xref:deploy/deploying-mule-to-weblogic#[Deploying Mule to WebLogic]
* xref:deploy/deploying-mule-to-websphere#[Deploying Mule to WebSphere]
* xref:deploy/deploying-mule-as-a-service-to-tomcat#[Deploying Mule as a Service to Tomcat]
* link:http://geronimo.apache.org[Geronimo] (The Geronimo application server uses ActiveMQ as its default JMS provider. For details, see xref:transports/activemq-integration#[ActiveMQ Integration].)

For details on how to support Mule hot deployment within some application servers, see xref:deploy/application-server-based-hot-deployment#[Application Server Based Hot Deployment].
...
Mule fully integrates with Spring, allowing you to take advantage of Spring's many features, including JNDI and EJB session beans. You can also use Spring remoting to access Mule from an external application. For details, see xref:spring-integration/using-mule-with-spring#[Using Mule with Spring].
