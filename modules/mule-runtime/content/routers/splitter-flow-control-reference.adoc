...
:page-id: mule-runtime@3.7::routers/splitter-flow-control-reference
...
[NOTE]
Weigh the pros and cons of using a splitter and aggregator in your flow, versus using a "Foreach" scope xref:scopes/foreach#using-foreach-versus-splitter-and-aggregator-pairs[here].
...
+
. Run the Mule project.
. Send the HTTP connector an HTTP request that includes a body with an attached XML file. MuleSoft recommends using a browser extension such as link:https://chrome.google.com/webstore/detail/postman-rest-client/fdmmgilgnpjigdojojpjoooidkmcomcm[Postman] (Google Chrome).
...
+
You can look at the values of these outbound variables by putting a break point after the splitter and running your flow with the xref:anypoint-studio@5::studio-visual-debugger#[Visual Debugger]:
...
* Learn more about xref:mel/mule-expression-language-mel#[Mule Expression Language (MEL)].
* Learn about other xref:routers/all-flow-control-reference#[Flow Control] elements.
* Read more about the xref:scopes/async-scope-reference#[Async Scope].
