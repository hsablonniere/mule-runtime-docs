...
:page-id: mule-runtime@3.8::routers/scatter-gather
...
This document assumes you are familiar with Mule applications and link:/mule-user-guide/v/3.8/message-processors[message processors], in particular xref:routers/routers#[routing message processors].
...
|*Threading Profile* |Optionally, use to customize the threading profile. Refer to xref:administer/tuning-performance#[Tuning Performance] for a description of the configurable attributes. For a brief discussion of threading profiles in Scatter-Gather, see the section below. Default is `Default threading profile`. |
...
* Learn more about xref:general@master::content-based-routing#[Content-Based Routing]
