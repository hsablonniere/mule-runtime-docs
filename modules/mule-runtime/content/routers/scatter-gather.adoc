...
:page-id: mule-runtime@3.7::routers/scatter-gather
...
This document assumes you are familiar with Mule applications and link:/mule-user-guide/v/3.7/message-processors[message processors], in particular xref:routers/routers#[routing message processors].
...
|*Threading Profile* |Optionally, use to customize the threading profile. Refer to xref:administer/tuning-performance#[Tuning Performance] for a description of the configurable attributes. For a brief discussion of threading profiles in Scatter-Gather, see the section below. + |Default threading profile |
...
|===
|Optional Child Element |Description
|*threading-profile* |Allows you to configure the underlying thread pool. Refer to xref:administer/tuning-performance#[Tuning Performance] for a list of configurable attributes, all of which can be applied here. For a brief discussion of threading profiles in Scatter-Gather, see the section below.
|===
=====
...
For details on setting up threading profiles, see xref:administer/tuning-performance#[Tuning Performance].
