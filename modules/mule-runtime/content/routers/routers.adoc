...
:page-id: mule-runtime@3.8::routers/routers
...
|xref:apikit@3.8.3::about-apikit#[APIkit Router]| Based on an API link:https://raml.org[RAML file], it routes arriving calls to the corresponding flow depending on the resource and method. See xref:apikit@3.8.3::about-apikit#[APIkit documentation].
...
|xref:scopes/until-successful-scope#[Until Successful] |Repeatedly attempt to process a message until successful
|xref:routers/scatter-gather#[Scatter Gather] |Sends a request message to multiple targets concurrently. It collects the responses from all routes, and aggregates them into a single message.
|xref:apikit@3.8.3::apikit-for-soap#[SOAP Router]| Based on a WSDL file, it routes arriving calls to the corresponding flow depending on the resource and method. See xref:apikit@3.8.3::apikit-for-soap#[APIkit for SOAP documentation].
...
xref:routers/choice-flow-control-reference#[Read More]
...
[NOTE]
====
This Aggregator and others makes use of two object stores, one that buffers the messages as they arrive, and another that keeps track of the completed collections. You can use the default built in object stores, or you can reference your own custom built xref:modules/mule-object-stores#[Object Stores]. To do so, add the corresponding property to your aggregator element in XML and point to the location of the object store to use.
...
*From 3.1.0* you can further customize the behavior of this router by specifying a _'failureExpression'_ that allows you to use xref:non-mel-expressions/non-mel-expressions-configuration-reference#[Mule Expressions] to define a failure. The _failureExpression_ attribute is configured as follows:
...
xref:routers/splitter-flow-control-reference#[Read More]
