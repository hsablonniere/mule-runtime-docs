...
:page-id: mule-runtime@3.8::administer/mule-agents
...
[NOTE]
This document refers to the Agent that communicates Mule Runtimes to the xref:mule-management-console@3.8::about-mmc#[Mule Management Console], not to the one that communicates Mule Runtimes to the xref:anypoint-platform@latest::runtime-manager:index#[Runtime Manager]
...
Mule provides xref:administer/jmx-management#[several agents for JMX support], including notifications and remote management. You can also create custom agents to plug any functionality into Mule, such as running functionality as a background process or embedding a server in Mule.
...
For a list of agents provided with Mule and how to configure them, see xref:administer/jmx-management#[JMX Management]. You can also create a custom agent as described below.
