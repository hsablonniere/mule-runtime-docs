...
:page-id: mule-runtime@3.7::mule-configuration/understanding-mule-configuration
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see xref:components/entry-point-resolver-configuration-reference#[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

❾ Calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about connectors, see xref:connectors:anypoint-connectors#[Anypoint Connectors].
...
❶❺ Writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see xref:exceptions/error-handling#[Error Handling].
...
Mule allows you to define Agents to extend the functionality of Mule. Mule will manage the agents' lifecycle (initialize them and start them on startup, and stop them and dispose of them on sutdown). These agents can do virtually anything; the only requirement is that they implement `org.mule.api.agent.Agent`, which allows Mule to manage them.  For more about Mule agents, see xref:administer/mule-agents#[Mule Agents].
