...
:page-id: mule-runtime@3.8::mule-configuration/understanding-mule-configuration
...
For more detail see xref:mule-configuration/about-the-xml-configuration-file#[About the XML Configuration File].
...
<2> Shows the namespace for Mule's xref:transports/stdio-transport-reference#[stdio transport], which allows communication using standard I/O. It has the prefix "stdio". The convention for a Mule module or transport's schema is to use its name as the prefix. +
...
For more about Mule transformers, see xref:transformers/using-transformers#[Using Transformers].
...
Filters once again can be configured as global elements and referred to where they are used, or configured at their point of use.  For more information, see xref:filters/filters#[Filters].

==== Expressions

For a current reference to using expressions in Mule, see xref:mel/mule-expression-language-mel#[Mule Expression Language MEL]. 
...
Message enrichment is done using the `enricher` element. Unlike message transformation, which alters the current message's payload, enrichment adds additional properties to the message. This allows the flow to build up a collection of information for later processing.  For more about enriching messages see xref:scopes/message-enricher#[Message Enricher].
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see xref:components/logger-component-reference#[Logger Component Reference]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see xref:routers/routers#[Routing Message Processors].
...
<1> and <2> are Java components, specified by each's class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see xref:components/entry-point-resolver-configuration-reference#[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

In #9 in <<Flows>>, the example calls `authorize.net` to authorize a credit card purchase, passing it information from the message.  For more about connectors, see xref:connectors:anypoint-connectors#[Anypoint Connectors].
...
In #14 in <<Flows>>, the example writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see xref:exceptions/error-handling#[Error Handling].
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see xref:transactions/transaction-management#[Transaction Management].
...
For more about Mule security, see xref:security/configuring-security#[Configuring Security].
...
For more about notifications, see xref:mule-configuration/notifications-configuration-reference#[Notifications Configuration Reference].

=== Agents

Mule allows you to define Agents to extend the functionality of Mule. Mule manages the agents' lifecycle (initializes them and starts them on startup, and stops them and disposes of them on shutdown). These agents can do virtually anything; the only requirement is that they implement `org.mule.api.agent.Agent`, which allows Mule to manage them.  For more about Mule agents, see xref:administer/mule-agents#[Mule Agents].
