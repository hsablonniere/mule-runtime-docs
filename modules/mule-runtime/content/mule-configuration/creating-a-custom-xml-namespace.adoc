...
:page-id: mule-runtime@3.7::mule-configuration/creating-a-custom-xml-namespace
...
Other namespaces can be introduced via `<spring:beans>` or by adding a dedicated element in a module (see the xref:modules/scripting-module-reference#[Scripting module's] `<lang>` element).
...
The definition parser is where the actual object reference is created. It includes some Spring-specific classes and terminology, so it's worth reading link:http://spring.io/blog/2006/08/28/creating-a-spring-2-0-namespace-use-spring-s-abstractbeandefintionparser-hierarchy[this introduction].
