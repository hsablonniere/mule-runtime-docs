...
:page-id: mule-runtime@3.8::batch/batch-processing
...
To build applications with Batch jobs and run them in a test or production environment, you must build the application with an Enterprise runtime. Learn more about xref:anypoint-studio@6::mule-runtime/changing-runtimes-in-studio#[Changing Runtimes in Studio].
====
...
* Anypoint Studio. See xref:anypoint-studio@6::about-anypoint-studio#[Anypoint Studio Essentials] chapter to learn more about developing with Mule's graphical user interface.
* xref:connectors:anypoint-connectors#[Anypoint Connectors].
* xref:anypoint-studio@6::datamapper/datamapper-user-guide-and-reference#[Anypoint DataMapper User Guide and Reference]
transformer that you can use in applications in Studio. Note: DataMapper has been replaced by
xref:dataweave/dataweave#[DataWeave].
* xref:mule-configuration/global-elements#[Global Elements]. 
* xref:anypoint-studio@6::datasense/datasense#[DataSense].
* xref:scopes/poll-reference#[Polls]. 
...
When it comes to a batch job, all records are first stored in a persistent queue before the xref:batch/batch-processing#process[processing phase] begins, so the traditional threading model wouldn't apply.
...
Access xref:batch/batch-processing-reference#[Batch Processing Reference] for more information.
====
...
* A *batch job instance* is an occurrence in a Mule application whenever a Mule flow executes a batch job. Mule creates the batch job instance in the <<Load and Dispatch>> phase. Every _batch job instance_ is identified internally using a unique String known as xref:batch/batch-job-instance-id#[batch job instance id].
====
...
Since Batch was designed to work with millions of records, they need to be stored in a persistent storage queue. However, consider having 1 million records to place in a queue for a 3 step batch job. At least three million I/O operations occur as Mule takes and requests each record as it moves through the xref:batch/batch-processing#batch-processing-phases[job's phases].
...
. Mule sends the message payload through a xref:routers/splitter-flow-control-reference#[collection splitter]. This first step triggers the creation of a new batch job instance.
...
* You can set *record variables* on records and pass them from step to step (xref:batch/record-variable#[read more])
...
* You can *commit* records in groups, sending them as bulk upserts to external sources or services. (xref:batch/batch-filters-and-batch-commit#[read more])
...
--
+
. *Continue processing* the batch regardless of any failed records, using xref:batch/batch-filters-and-batch-commit#[filters] to instruct subsequent batch steps how to handle failed records
...
Read more about xref:batch/batch-filters-and-batch-commit#[fine-tuning filters] on batch steps to manage failed records at a more granular level.


Also check link:https://docs.mulesoft.com/mule-user-guide/v/3.8/using-mel-with-batch-processing#mel-functions-that-involve-batch-jobs[MEL expressions for Batch Processing] for a detailed description of the MEL expressions you can use to monitor
...
If a batch job _does not_ accumulate enough failed records to cross the `max-failed-records` threshold, _all_ records – successes and failures – continue to flow from batch step to batch step; use xref:batch/batch-filters-and-batch-commit#[filters] to control which records each batch step processes.
...
[TIP]
Note that this example introduces features not discussed in great detail in this document. Consult xref:batch/batch-filters-and-batch-commit#[Batch Filters and Batch Commit] and xref:batch/record-variable#[Record Variable] for more information.
...
[start=3]
. Mule begins processing each lead as a record. The first batch step, `lead-check`, uses a *Salesforce Connector* wrapped with a xref:scopes/message-enricher#[Message Enricher] to:
...
* Batch processing does not support the use of xref:mule-configuration/business-events#[Business Events].
* xref:anypoint-platform@latest::cloudhub:insight#[Insight] does not support visibility into batch processing.
...
* Access xref:batch/batch-processing-reference#[reference details] about batch processing.
* Examine the xref:batch/batch-processing-reference#[attributes] you can configure for batch jobs, steps and message processors.
...
* Learn more about xref:batch/batch-processing-reference#[MEL expressions] you can use to access batch processing data.
* Read more about xref:anypoint-platform@latest::cloudhub:cloudhub-fabric#[CloudHub support for batch processing].
* Learn more about xref:connectors:anypoint-connectors#[Anypoint Connectors].
* Learn more about xref:scopes/poll-reference#[Polling and Watermarks].
* Learn more about xref:dataweave/dataweave#[DataWeave].
