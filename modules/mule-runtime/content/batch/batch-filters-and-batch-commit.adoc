...
:page-id: mule-runtime@3.8::batch/batch-filters-and-batch-commit
...
This document assumes that you are familiar with xref:anypoint-studio@6::about-anypoint-studio#[Anypoint Studio], have reviewed the overview and basic anatomy of xref:batch/batch-processing#[Batch Processing], and that you understand how batch jobs handle xref:batch/batch-processing#[records which fail processing]. You should also consider xref:batch/batch-streaming-and-job-execution#streaming-batch-commits[Streaming Batch Commits].
...
*Note:* See also xref:batch/batch-streaming-and-job-execution#streaming-batch-commits[Streaming Batch Commits].
...
In previous versions of Mule, the commit block exposed only the grouped record’s payload and didn't allow you to change those payloads nor to retrieve the associated xref:batch/record-variable#[record variables].
...
During the xref:batch/batch-processing#process[process phase] of your batch job, you can replace, change, or store the payload data.

By adding a xref:scopes/foreach#[Foreach Scope] you can iterate trough a fixed size commit block, and use the xref:components/expression-component-reference#[Expression Component Reference] to modify the payload and create a record variable for each collected record.
...
* xref:batch/batch-processing#[Batch Processing] for a full description of the steps the batch job takes in each phase of processing.
...
* Read about the xref:batch/batch-processing#[basic anatomy] of batch processing in Mule.
