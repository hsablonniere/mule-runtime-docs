...
:page-id: mule-runtime@3.7::modules/enabling-ws-security
...
For more information, see link:https://www.mulesoft.com/exchange#!/soap-web-service-security[SOAP Web Service Security] in Anypoint Exchange. You can also use the xref:connectors:web-service-consumer/web-service-consumer#[Web Service Consumer] to provide username token and timestamp support for WS-Security.
...
To the Mule flow that involves a Web service, add a xref:components/cxf-component-reference#[CXF Component] to publish, consume, or proxy a Web service. Complete the following steps to configure WS-security on the SOAP component.
...
link:http://www.mulesoft.org/schema/mule/cxf/3.7/mule-cxf.xsd[View the Mule 3.7 CXF schema].
...
* See link:https://www.mulesoft.com/exchange#!/?types=example[examples].
