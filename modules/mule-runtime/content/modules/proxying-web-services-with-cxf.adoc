...
:page-id: mule-runtime@3.8::modules/proxying-web-services-with-cxf
:keywords: cxf
...
TIP: Instead of building a proxy application from scratch, consider xref:anypoint-platform@latest::api-manager:setting-up-an-api-proxy#[Setting Up and Deploying a Proxy].

== Deciding How to Proxy Your Web Service

While many times you can proxy web services without using CXF (see xref:api-usage/proxying-web-services#[Proxying Web Services]), there are several cases where you might want to use CXF proxies:
...
[TIP]
To send outbound requests to external web services, it's highly recommended that you use the xref:connectors:web-service-consumer/web-service-consumer#[Web Service Consumer] instead.
