...
:page-id: mule-runtime@3.7::modules/json-module-reference
...
Mule supports binding JSON data to objects and marshalling Java object to JSON using the link:https://github.com/codehaus/jackson[Jackson] Framework. Jackson uses annotations to describe how to map data to a Java object model. For example, use JSON data to map a person's information:
...
You can use JsonPath in xref:mel/mule-expression-language-mel#[Mule expressions] to query JSON message payloads for filtering or xref:scopes/message-enricher#[enrichment].
