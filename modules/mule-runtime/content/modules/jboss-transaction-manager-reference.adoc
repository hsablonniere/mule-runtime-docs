...
:page-id: mule-runtime@3.7::modules/jboss-transaction-manager-reference
:keywords: mule, studio, jboss, bpms

This module enables Mule to use the JBoss transaction manager (previously Arjuna) to configure xref:transactions/xa-transactions#[XA transactions]. Developers can configure one Transaction Manger per Mule instance. For more information, see link:http://www.jboss.org/jbosstm/[JBoss Transactions], xref:deploy/deploying-mule-to-jboss#[Deploying Mule to JBoss], xref:transactions/transaction-management#[Transaction Management], xref:modules/jboss-jbpm-module-reference#[JBoss jBPM Module Reference].
