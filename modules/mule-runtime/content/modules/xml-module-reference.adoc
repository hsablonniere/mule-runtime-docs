...
:page-id: mule-runtime@3.8::modules/xml-module-reference
...
In addition to the functionality described on this page, you can also use the xref:modules/sxc-module-reference#[SXC Module Reference], which enables efficient XPath XML routing.
...
- `xpath2()` function, superseded by the xref:modules/xpath#[xpath3()] function +
- `JXPath` extractor +
- `JXPath` filter +
- `Jaxen` filter +

New function: xref:modules/xpath#[xpath3()], which includes improved return type handling. For details, see xref:modules/xpath#[XPath]. +
*Version 1* - Not Supported +
*Version 2* - Supported +
*Version 3* - Supported
|XQuery
|The xref:modules/xslt-transformer#[XSLT Transformer] now supports XSLT 3.0. The transformer's behavior and syntax remain unaltered, and you can manually select which XSLT version your transformer should use. For details, see the xref:modules/xslt-transformer#[XSLT Transformer] documentation. +
*Version 1* - Supported +
*Version 2* - Supported +
*Version 3* - Supported
|XSLT
|The xref:modules/xquery-transformer#[XQuery Transformer] now supports XQuery 3.0. The transformer's behavior and syntax remain unaltered, and you can manually select which XQuery version your transformer should use. For details, see xref:modules/xquery-transformer#[XQuery Transformer] and xref:modules/xquery-support#[XQuery Support]. +
*Version 1* - Supported +
*Version 2* - Supported +
*Version 3* - Supported
|===
...
|xref:modules/jxpath-extractor-transformer#[JXPath Extractor] _*Deprecated*_ |Queries and extracts object graphs using XPath expressions using JXPath. Deprecated in favor of the `xpath3()` function (see xref:modules/xpath#[XPath]).
...
The XML module contains various XPath filters. For general details on how to use filters, see xref:filters/filters#[Filters].

=== XPath Filter

[NOTE]
Mule 3.6.0 and later versions support XPath 3.0, which is backwards-compatible with XPath 2.0. On the other hand, XPath 1.0 is deprecated, and while simple expressions may work, some expressions may be incompatible. For details, see xref:modules/xpath#[XPath].
...
For a detailed description of the `xpath3` function, see xref:modules/xpath#[XPath].
====
