...
:page-id: mule-runtime@3.7::modules/modules-reference
:keywords: anypoint studio, esb, modules

Modules are similar to xref:transports/connecting-using-transports#[transports] in that they provide pluggable functionality, configured via dedicated schema, but they differ in that there is no underlying transport to send or receive data. Following is a list of the default Mule ESB modules.
...
|xref:security/saml-module#[SAML Module] _*Enterprise Edition*_ |Provides authentication and authorization capabilities to Mule based on the SAML standard. (As of Mule enterprise edition 2.2.3)
