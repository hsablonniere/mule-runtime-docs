...
:page-id: mule-runtime@3.7::modules/jersey-module-reference
...
Our recommended approach to REST APIs is using link:http://raml.org[RAML] and exposing APIs through the xref:anypoint-platform@latest::api-manager:about-api-manager#[API Manager]. Jersey might still in some cases be the path of minimum resistance for when the API you want to expose was built in Java and lacks a RAML file that describes how to interface to it.
