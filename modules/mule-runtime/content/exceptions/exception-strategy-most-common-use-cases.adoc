...
:page-id: mule-runtime@3.8::exceptions/exception-strategy-most-common-use-cases
...
. The catch exception strategy replaces the message payload with new content. Mule uses the xref:mel/mule-expression-language-mel#[Mule Expression Language MEL] `exception` variable to include an error number in the message payload.
