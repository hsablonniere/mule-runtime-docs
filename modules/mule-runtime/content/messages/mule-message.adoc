...
:page-id: mule-runtime@3.7::messages/mule-message
...
If you completed the xref:anypoint-studio@6::basic-studio-tutorial#[Basic Studio Tutorial] or the xref:general@master::content-based-routing#[How to Route Mule Messages Tutorial], you have already encountered and used some MEL expressions. Work through this tutorial to orient yourself further to MEL and how you can use it within your Mule applications to extract information from your message, set or manipulate values, or perform operations based on data extracted from the message at runtime.

== Prerequisites

This tutorial assumes that you have downloaded and installed link:https://www.mulesoft.com/platform/studio[Anypoint Studio]. If you do not have any previous experience with Eclipse or an Eclipse-based IDE, please review the brief introduction to the xref:anypoint-studio@6::about-anypoint-studio#[Anypoint Studio interface] or complete the xref:anypoint-studio@6::basic-studio-tutorial#[Basic Studio Tutorial]. 
...
This tutorial also assumes that you have a basic understanding of the xref:@3.8::messages/about-mule-message#[Mule message structure].
...
. Launch Anypoint Studio and create a new Mule Project. If you need more directions on launching Studio and creating a project, please refer to the xref:anypoint-studio@6::basic-studio-tutorial#[Basic Studio Tutorial]. 
...
The most direct and dynamic way of exploring which MEL expressions resolve as you expect them to is to use the Mule Expression Evaluator within the xref:anypoint-studio@6::studio-visual-debugger#[Visual Debugger]. You can access the Mule Expression Evaluator while running in Debug mode, after having sent a message and paused it at a breakpoint in your flow.
...
For more ideas on MEL expressions to test out in the Mule Expression Evaluator, refer to the xref:@3.8::mel/mule-expression-language-basic-syntax#[syntax guide] or borrow some from the collection of introductory xref:@3.8::mel/mule-expression-language-examples#[MEL examples].
...
+
image:mmt-extra-credit-flow.png[mmt-extra-credit-flow]
+
...
* *NEXT STEP:* Learn how to xref:@3.8::deploy/starting-and-stopping-mule-esb#[start and stop Mule] from the command line.
* Refer to the main xref:@3.8::mel/mule-expression-language-mel#[MEL documentation] including the collection of xref:@3.8::mel/mule-expression-language-examples#[examples], which demonstrate how to use MEL expressions in a variety of other message processors.
* Access the Mule Expression Language xref:@3.8::mel/mule-expression-language-reference#[reference] page for a searchable listing of context objects and their fields, operations, and functions. 
* Learn more about xref:anypoint-studio@6::studio-visual-debugger#[Visual Debugger], the xref:@3.8::components/logger-component-reference#[Logger], and xref:anypoint-studio@6::datasense/datasense#[DataSense].
* If you haven't already, try some of our other tutorials, all of which involve using MEL expressions:
** xref:anypoint-studio@6::basic-studio-tutorial#[Basic Studio Tutorial]
** xref:general@master::content-based-routing#[Content-Based Routing Tutorial]
** xref:general@master::anypoint-connector#[Anypoint Connector quickstart]
