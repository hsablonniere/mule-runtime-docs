...
:page-id: mule-runtime@3.8::messages/mule-message
...
This tutorial also assumes that you have a basic understanding of the xref:messages/about-mule-message#[Mule message structure].
...
For more ideas on MEL expressions to test out in the Mule Expression Evaluator, refer to the xref:mel/mule-expression-language-basic-syntax#[syntax guide] or borrow some from the collection of introductory xref:mel/mule-expression-language-examples#[MEL examples].
...
* *NEXT STEP:* Learn how to xref:deploy/starting-and-stopping-mule-esb#[start and stop Mule] from the command line.
* Refer to the main xref:mel/mule-expression-language-mel#[MEL documentation] including the collection of xref:mel/mule-expression-language-examples#[examples], which demonstrate how to use MEL expressions in a variety of other message processors.
* Access the Mule Expression Language xref:mel/mule-expression-language-reference#[reference] page for a searchable listing of context objects and their fields, operations, and functions. 
* Learn more about xref:anypoint-studio@6::studio-visual-debugger#[Visual Debugger], the xref:components/logger-component-reference#[Logger], and xref:anypoint-studio@6::datasense/datasense#[DataSense].
