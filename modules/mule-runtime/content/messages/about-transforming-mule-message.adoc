...
:page-id: mule-runtime@3.7::messages/about-transforming-mule-message
...
This document uses an example application and draws upon the content of the xref:messages/about-mule-message#[Mule Message Structure document] to examine the state of a message as it passes through a xref:dataweave/dataweave#[Transform Message component] in a flow. Running the application in debug mode in Anypoint Studio, the screenshots illustrates the state of a message using the xref:anypoint-studio@5::studio-visual-debugger#[Visual Debugger] which facilitates "frozen-in-time" viewing of a message.

== Assumptions

This document describes the details of the example within the context of *Anypoint Studio* , Mule ESB’s graphical user interface (GUI). Where appropriate, the XML configuration accompanies the Studio interface screenshots. This document assumes that you are familiar with Mule ESB and the  xref:anypoint-studio@5::about-anypoint-studio#[Anypoint Studio interface] . Further, it assumes you have read the xref:messages/about-mule-message#[Mule Message Structure document] to understand the contents of a Mule message. 

== Example

To examine the state of a message, this document uses an example application which converts the contents of a CSV file into contacts in Salesforce. You can view examples in xref:anypoint-platform@latest::exchange:anypoint-exchange#[the Library].
...
Learn more about how to examine the state of your messages using the xref:anypoint-studio@5::studio-visual-debugger#[Visual Debugger] .
====
...
*  Learn more about xref:anypoint-studio@5::studio-visual-debugger#[Studio Visual Debugger].