...
:page-id: mule-runtime@3.8::transports/multicast-transport-reference
:keywords: anypoint studio, connector, endpoint, multicast, transport

The link:http://en.wikipedia.org/wiki/Multicast[Multicast] transport allows sending messages to or receiving messages from groups of multicast sockets. It is implemented on top of xref:transports/udp-transport-reference#[UDP] and is highly scalable since knowledge of the receivers is not required.
...
IP Multicasting is a service provided by IP (the internet protocol layer), that allows one-to-many communication. The most common use of IP Multicasting is to send UDP datagrams to multiple sockets located on different systems. Mule supports this with the Multicast transport. Note that, except for the communication being many-to-one instead of one-to-one, the Multicast transport is very similar to the xref:transports/udp-transport-reference#[UDP transport] and the same considerations should be observed.
...
link:http://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/transport/multicast/package-summary.html[Multicast]
