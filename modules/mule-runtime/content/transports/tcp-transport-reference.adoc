...
:page-id: mule-runtime@3.8::transports/tcp-transport-reference
...
* The xref:transports/ssl-and-tls-transports-reference#[SSL and TLS Transports], which use TCP with socket-level security. 
...
|Doc |link:http://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/transport/multicast/package-summary.html[JavaDoc] +
...
|Maven Artifact |org.mule.transport:mule-transport-multicast |The group name a artifact name for this transport in link:http://maven.apache.org/[Maven].
|===
...
* Are you communicating with a flow always located in the same Mule application instance? If so, consider use the xref:transports/vm-transport-reference#[VM transport].
...
* Should messages be secured? If so, use the xref:transports/ssl-and-tls-transports-reference#[SSL transport].
