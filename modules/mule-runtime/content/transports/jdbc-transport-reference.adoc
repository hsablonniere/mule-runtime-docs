...
:page-id: mule-runtime@3.7::transports/jdbc-transport-reference
:keywords: jdbc
...
Access the documentation for the xref:connectors:database/database-connector#[Database Connector] to take advantage of the newly-designed functionality.
====
...
|JDBC |http://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/transport/jdbc/package-summary.html[JavaDoc] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)](local,XA) |image:error.png[(error)] |image:check.png[(tick)] |one-way, request-response |one-way |org.mule.http://transportmule-transport-jdbc[transport:mule-transport-jdbc]
...
Note that stored procedures are only supported on outbound endpoints. If you want to set up a flow that calls a stored procedure at a regular interval, you can define a xref:transports/quartz-transport-reference#[Quartz] inbound endpoint and then define the stored procedure call in the outbound endpoint. For information on using Quartz to trigger flows, see the following link:http://blogs.mulesoft.com/dev/mule-dev/using-quartz-to-trigger-a-service/[blog post].
...
Transactions are supported on JDBC endpoints. See xref:transactions/transaction-management#[Transaction Management] for details.
