...
:page-id: mule-runtime@3.8::transports/ssl-and-tls-transports-reference
:keywords: anypoint studio, ssl, tls, transports

[WARNING]
This page refers to using SSL and TLS together with the old endpoint based
xref:connectors:http/http-connector#[HTTP connector]. For using TLS with the new HTTP Connector and the
xref:connectors:web-service-consumer/web-service-consumer#[Web Service Consumer] connector, see
xref:connectors:tls/tls-configuration#[TLS].
...
|class |The name of the class that implements the custom protocol |  |See xref:transports/tcp-transport-reference#[below] for an example of writing a custom protocol
