...
:page-id: mule-runtime@3.7::transports/ssl-and-tls-transports-reference
:keywords: anypoint studio

[WARNING]
This page refers to using SSL and TLS together with the old endpoint based link:https://developer.mulesoft.com/docs/display/current/HTTP+Connector[HTTP connector]. For using TLS with the new HTTP Connector and the xref:connectors:web-service-consumer/web-service-consumer#[Web Service Consumer] connector, see xref:connectors:tls/tls-configuration#[TLS Configuration].


The following Mule transports provide access to TCP connections:

* The xref:transports/tcp-transport-reference#[TCP Transport], which uses the basic TCP transport.
...
* Communicating with an external service that uses low-level unsecured TCP connections? If so, use the xref:transports/tcp-transport-reference#[TCP] protocol.

* Are you communicating with a flow always located in the same Mule application instance? If so, consider using the xref:transports/vm-transport-reference#[VM transport].

* Is it important that messages be persisted until they can be processed? If so, consider using a persistent transport like xref:transports/jms-transport-reference#[JMS] or xref:transports/file-transport-reference#[File].

* Are there advantages to a higher-level protocol built on top of TCP, for instance, the request-response features of xref:transports/deprecated-http-transport-reference#[HTTP], or the store-and-forward features of xref:transports/email-transport-reference#[Email]? If so, use the transport for that protocol instead.

* Is performance the primary concern and it is not important that messages be delivered in the proper order or that the sender is notified if any are lost? If so, use the lighter-weight xref:transports/udp-transport-reference#[UDP transport] instead.
...
|===
|Name |Notes
|class |See xref:transports/tcp-transport-reference#[TCP Transport Reference] for an example of writing a custom protocol
...
The SSL transport can be used for secure socket communication using SSL or TLS. The Javadoc for this transport can be found link:http://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/transport/ssl/package-summary.html[here].
...
Reference the link:http://www.mulesoft.org/docs/site/3.7.0/apidocs/[SSL Javadoc] for this module.
