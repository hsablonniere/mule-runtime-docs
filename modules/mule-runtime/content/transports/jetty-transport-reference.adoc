...
:page-id: mule-runtime@3.7::transports/jetty-transport-reference
:keywords: mule, studio, jetty, SSL

The Jetty transport provides support for exposing applications over HTTP by embedding a light-weight Jetty server. The xref:transports/jetty-ssl-transport#[Jetty SSL Transport] works the same way but over SSL. You can only define inbound endpoints with this transport.
...
Jetty endpoints are configured the same way as xref:transports/file-transport-reference#[FTP endpoints]. Note that the Jetty transport can only be used for inbound endpoints.
