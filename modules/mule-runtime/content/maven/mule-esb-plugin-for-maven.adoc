...
:page-id: mule-runtime@3.7::maven/mule-esb-plugin-for-maven
...
The muleesb-maven-plugin allows you to control Mule ESB instances from within a Maven environment. It is part of the framework for developing Mule applications with Maven, an overview of which you can find in xref:maven/using-maven-with-mule#[Using Maven with Mule]. The plugin allows you to:
...
[NOTE]
For the above example to work, you need to have the Mule ESB repository defined in your `settings.xml` or your `pom.xml`. For details, see xref:maven/using-maven-with-mule#[Using Maven with Mule] and xref:maven/configuring-maven-to-work-with-mule-esb#[Configuring Maven to Work with Mule ESB].
...
* For more information about developing Mule applications with Maven, see the overview in xref:maven/using-maven-with-mule#[Using Maven with Mule], and the guides at  xref:anypoint-studio@5::maven/building-a-mule-application-with-maven-in-studio#[Building a Mule Application with Maven in Studio].
* Learn how to xref:anypoint-studio@5::maven/importing-a-maven-project-into-studio#[import an existing Maven project into Anypoint Studio].
* Access additional Maven xref:maven/maven-reference#[reference] and xref:maven/configuring-maven-to-work-with-mule-esb#[configuration] information.
