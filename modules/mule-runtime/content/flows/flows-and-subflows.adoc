...
:page-id: mule-runtime@3.8::flows/flows-and-subflows
...
* In clusters of Mule servers, messages can migrate between nodes when sent to an asynchronous flow. This allows for *load balancing between nodes* and higher performance of application. (See our xref:flows/flow-architecture-advanced-use-case#[advanced use case] for an example.)
...
Read more about setting up xref:scopes/transactional#[Transactional units] in Mule applications.
====
...
|Asynchronous Flow |Flow Reference wrapped within an xref:scopes/async-scope-reference#[Async Scope] |asynchronous |not inherited
|===

== See Also

* *NEXT STEP:* Learn the various ways you can xref:general@master::to-get-anypoint-platform#[deploy Mule applications].
* Examine an xref:flows/flow-architecture-advanced-use-case#[advanced use case] showing a more complex flow architecture that uses several child flows.
* Read about some alternative ways to control message processing within a flow using xref:routers/routers#[routing message processors].
* Refer to the xref:flows/flow-reference-component-reference#[Flow Reference Component Reference] and xref:scopes/async-scope-reference#[Async Scope].
* Read more about xref:flows/flow-processing-strategies#[Flow Processing Strategies].
* Read more about setting up xref:scopes/transactional#[transactional units] in Mule applications.
