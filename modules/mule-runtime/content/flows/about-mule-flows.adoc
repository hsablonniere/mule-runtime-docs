...
A typical example of a message source is an xref:connectors:anypoint-connectors#[*Anypoint Connector*]. 
...
Under certain conditions, flows do not need to be triggered by message sources. For instance, a xref:flows/flow-reference-component-reference#[Flow Reference Component] can trigger a different flow in the same application. Similarly, the xref:scopes/async-scope-reference#[Async Scope] can trigger a another flow that executes asynchronously, (that is, in parallel with the originating flow).
...
For further details, see xref:flows/flow-processing-strategies#[Flow Processing Strategies].
...
Studio provides four pre-packaged error handling strategies to handle exceptions thrown at various points during the message processing sequence. For details, see xref:exceptions/error-handling#[Error Handling].
...
Although flows are very flexible, you cannot place just _any_ building block at _any_ position within a flow. The proximity or absence of certain building blocks within a sequence can determine whether a given building block can be placed at a certain point within a flow. Finally, depending where it resides in a flow, a given building block, especially an endpoint, can expose an significantly different set of attributes for configuration.

If you choose to develop using the visual editor in Anypoint Studio, Studio keeps track of all these contingencies, and it does not let you place a building block icon where it is not allowed.
...
* Call various other xref:flows/flows-and-subflows#[flows]

== See Also

* *NEXT STEP:* Learn about how xref:flows/flows-and-subflows#[flows and subflows] work together in an application.
* Examine an xref:flows/flow-architecture-advanced-use-case#[advanced use case] showing a more complex flow architecture.
