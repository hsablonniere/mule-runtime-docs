...
:page-id: mule-runtime@3.7::flows/flow-processing-strategies
...
This page assumes that you are familiar with xref:mule-concepts#[Mule flows] and basic xref:flows/about-mule-flows#[application architecture].
...
In addition, some inbound endpoints can be configured to operate xref:transactions/transaction-management#[transactionally], regardless of their exchange pattern. If a flow is transactional, Mule applies a synchronous processing strategy.
...
* *Wrap a Flow:* You can wrap one or more message processors within your synchronous flow in an xref:scopes/async-scope-reference#[asynchronous scope] to define a processing block that Mule processes asynchronously with the main flow. You can also create an asynchronous flow by wrapping a flow-ref in an asynchronous scope. 

* *Subflow*: You can call out to a subflow (which always processes synchronously) or a synchronous flow from any flow, regardless of what processing strategy is applied to the triggering flow. The triggering flow waits until this synchronous processing block returns a message before proceeding with processing. See xref:flows/flows-and-subflows#[Flows and Subflows] for more information about subflows and synchronous flows.

* *Non-Blocking*: You can use the new <<Non-Blocking Processing Strategy>> strategy available in Mule 3.7 and newer to take advantage of NIO with the xref:connectors:http/http-connector#[HTTP Connector] and avoid having threads blocked while waiting for HTTP responses, which also requires less tuning. *Note*: Not all components are currently supported, unsupported components cause a flow to fall back to synchronous processing. 
...
|*Synchronous* |*No.* You cannot force a flow with a request-response exchange pattern and/or transactionality to be asynchronous. |*No*. You cannot fine-tune the synchronous processing strategy. You can, however, customize the inbound endpoint connector receiver xref:administer/tuning-performance#[threading profile]. | *Yes.* You can use an Async Scope or an asynchronous flow to cause Mule to process a selected block of message processors asynchronously. <<Creating an Asynchronous Processing Block>>. |*Yes.* <<Creating a Custom Processing Strategy>>.
|*Queued-Asynchronous* |*Yes.* You can override an implicitly selected queued-asynchronous processing strategy by explicitly declaring a synchronous processing strategy (or, in rare use cases, a different kind of asynchronous processing strategy) instead. <<Specifying a Processing Strategy>>. |*Yes.* You can fine-tune the queued-asynchronous processing strategy to meet your needs. <<Fine-Tuning a Queued-Asynchronous Processing Strategy>>. |*Yes.* A synchronous xref:flows/flows-and-subflows#[flow or subflow] processes a selected block of message processors synchronously, regardless of the processing strategy of the main flow. |*Yes.* <<Creating a Custom Processing Strategy>>.
|=====
...
[NOTE]
Note that in addition to fine-tuning the attributes of the queued-asynchronous processing strategy, you can also xref:administer/tuning-performance#[tune performance] of your Mule application by analyzing and configuring thread profiles.
...
. Wrap one or more processors in an xref:scopes/async-scope-reference#[async scope]
. Create an xref:flows/flows-and-subflows#[asynchronous flow] by wrapping a flow-ref element in an async scope so that the contents of the flow will be processed asynchronously with the triggering flow
...
For more information about configuring the asynchronous scope element, refer to the xref:scopes/async-scope-reference#[Async Scope reference].
...
* The flow's inbound endpoint must be notified of all errors that occur during the processing of the message. This situation is discussed further in link:/mule-management-console/v/3.7/reliability-patterns[Reliability Patterns].
...
* The only Message Source that supports non-blocking is the xref:connectors:http/http-listener-connector#[HTTP Connector Listener].
* The only Message Processor that supports non-blocking is the xref:connectors:http/http-request-connector#[HTTP Connector Request].
...
* Refer to the xref:scopes/async-scope-reference#[Async Scope Reference] for information on how to embed an asynchronous processing block in an otherwise synchronous flow.
* Review the different kinds of xref:flows/flows-and-subflows#[Flows and Subflows] in Mule.
* Read more about link:/mule-management-console/v/3.7/reliability-patterns[Reliability Patterns].
* Find out how to xref:administer/tuning-performance#[tune performance] by analyzing and configuring threading profiles. 
