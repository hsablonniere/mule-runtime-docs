...
:page-id: mule-runtime@3.8::mel/mule-expression-language-mel
...
This document assumes that you are familiar with Mule and its xref:mule-concepts#[basic concepts]. Consider reading the chapter, xref:general@master::to-get-anypoint-platform#[Anypoint Fundamentals] to gain familiarity before proceeding. 
...
* *NEXT STEP*: Learn the xref:mel/mule-expression-language-basic-syntax#[basic syntax of MEL], or dive into xref:flows/about-mule-flows#[Mule Application Architecture].
* Learn better by example? Access our collection of xref:mel/mule-expression-language-examples#[example apps which use MEL].
* Learn better by tutorial? Access the xref:messages/mule-message#[Mule Message Tutorial].
...
* Learn how to take advantage of DataWeave within MEL with xref:mel/mel-dataweave-functions#[MEL DataWeave Functions]
