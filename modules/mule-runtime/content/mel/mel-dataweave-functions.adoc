...
:page-id: mule-runtime@3.8::mel/mel-dataweave-functions
:keywords: mel, cheat, sheet

The xref:mel/mule-expression-language-mel#[Mule Expression Language (MEL)] includes two functions that take advantage of the powerful xref:dataweave/dataweave#[DataWeave] transformation language, ideal for carrying out complex data transformations.

All components in Mule that support Mule Expression Language also support expressions written in DataWeave Language.

[TIP]
The DataWeave language is easiest to use by adding a *Transform Message Component* to your flow, as it offers a graphical UI to build automatic mappings through a drag-and-drop interface, as well as rich debugging features. See xref:anypoint-studio@6::using-dataweave-in-studio#[Using DataWeave in Studio] for more details.

== Prerequisites

This document assumes you are familiar with both xref:mel/mule-expression-language-reference#[Mule Expression Language] and xref:dataweave/dataweave-language-introduction#[DataWeave Language].
...
|Reader Properties	| For inputs such as CSV, sometimes some properties are needed to know how to parse the input, see xref:anypoint-studio@6::using-dataweave-in-studio#reader-configuration[Reader Configuration]
|Writer Properties	| For inputs such as CSV, sometimes some properties are needed to compose the output, see xref:dataweave/dataweave-language-introduction#output-directive[Output Directive]

|===

[NOTE]
Reader and writer properties are only supported on versions xref:general@master::release-notes:mule-runtime/3.8.2-release-notes#[Mule Runtime 3.8.2 Release Notes] and newer.
...
That same expression could be added inside a xref:scopes/foreach#[Foreach Scope] in the *Collection* field, to use this expression to parse the payload into a separate set of messages to process individually (one at a time):
...
* xref:mel/mule-expression-language-reference#[Mule Expression Language Reference]
