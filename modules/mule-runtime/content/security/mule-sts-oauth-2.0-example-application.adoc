...
:page-id: mule-runtime@3.8::security/mule-sts-oauth-2.0-example-application
...
This document assumes that you are familiar with the xref:anypoint-studio@6::about-anypoint-studio#[Anypoint Studio interface]. To increase your familiarity with Anypoint Studio, consider completing one or more xref:general@master::to-get-anypoint-platform#[Anypoint Studio Quickstarts].
...
. If you do not already have *Mule Enterprise Standalone* installed on your system, download link:http://www.mulesoft.com/mule-esb-enterprise-30-day-trial[Mule Enterprise trial Runtime].
...
+
[IMPORTANT]
The `tweetbook-oauth2-provider` sample implements the `OAuth provider module` available in the xref:security/anypoint-enterprise-security#[Anypoint Enterprise Security] collection.
+
. If you don't have it installed, follow the steps detailed in xref:security/installing-anypoint-enterprise-security#[Installing Anypoint Enterprise Security].
...
Refer to xref:security/authorization-grant-types#[Authorization Grant Types] for more information on the other types of authorization grants.
...
* For more information on Oauth 2.0 in Mule, refer to xref:security/mule-secure-token-service#[Mule Secure Token Service].
