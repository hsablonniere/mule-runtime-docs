...
:page-id: mule-runtime@3.8::security/anypoint-enterprise-security-example-application
...
This document assumes that you have xref:security/installing-anypoint-enterprise-security#[installed the Anypoint Enterprise Security] extensions.

It also assumes you are familiar with the xref:anypoint-studio@6::about-anypoint-studio#[Anypoint Studio] interface. To increase your familiarity with Studio, consider completing one or more xref:general@master::to-get-anypoint-platform#[Anypoint Studio Quickstarts].
...
The *Encrypted Message Producer* application consists of three simple xref:flows/about-mule-flows#[flows], each of which acts as a Web service provider which encrypts or decrypts a message before returning a response to the caller. Organized according to function, these flows serve different purposes within the application.
...
Make sure to right click on any of the xref:security/mule-message-encryption-processor#[Mule Message Encryption Processors] and select `Add Encryption libraries to the project`.
...
* For more information on encryption in Mule, see xref:security/mule-message-encryption-processor#[Mule Message Encryption Processor].
* For more information on signatures in Mule, see xref:security/mule-digital-signature-processor#[Mule Digital Signature Processor].
