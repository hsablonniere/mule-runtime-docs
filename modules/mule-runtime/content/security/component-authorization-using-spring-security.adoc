...
:page-id: mule-runtime@3.7::security/component-authorization-using-spring-security
...
In addition to the `daoAuthenticationProvider` and `inMemoryDaoImpl` beans (see xref:security/configuring-security#[Configuring Security]), the following beans must be configured:
