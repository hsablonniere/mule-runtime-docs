...
Before you begin, read the xref:general@master::release-notes:mule-runtime/supported-versions#[release notes] for the Mule Runtime version you wish to upgrade to (which we will call the target version, from here on), to know if you must make any necessary changes to your applications to ensure compatibility. For example, if you migrate from a version that's older than 3.6 to a version that's newer, you might want to xref:@3.8::connectors:http/migrating-to-the-new-http-connector#[change all HTTP connectors] to the newer version of this connector as the old one is deprecated as of Mule Runtime 3.6.
...
. xref:anypoint-studio@6::install-update-items-in-studio#[Download the newest Mule runtime] on Anypoint Studio so that you can handle that version in the IDE.
+
[TIP]
You can check xref:anypoint-studio@6::mule-runtime/adding-community-runtime#[this documentation page], as a reference on how to install additional runtimes on Anypoint Studio.

. For every application that you update:
.. xref:anypoint-studio@6::import-export-projects-studio#exporting-a-mule-deployable-archive[Import the application] to Anypoint Studio as a Mule project.
...
.. xref:anypoint-studio@6::import-export-projects-studio#exporting-a-mule-deployable-archive[Export your project] as a Mule Deployable Archive (.zip file).
...
. xref:anypoint-studio@6::install-update-items-in-studio#[Download] and install the new Mule version (preferably on a different server or virtual machine).
...
[NOTE]
====
If any of your previous applications rely on a full or a partial custom patch in order to work properly (these would be found in the folder `<MULE_HOME>/lib/user`), please verify that the fix is included in this target release by reviewing the corresponding xref:general@master::release-notes:mule-runtime/supported-versions#[release notes] page.
...
====
+
. Download a new copy of your license from the Customer Portal and install it on the target version, by following the instructions described at xref:@3.8::install/installing-an-enterprise-license#[Installing an Entrerprise License].
...
. xref:@3.8::deploy/starting-and-stopping-mule-esb#[Run your new Mule Server] to verify your installation is working as expected.
...
[NOTE]
To migrate following these steps, you must have the xref:runtime-manager-agent@1.6.3::runtime-manager-agent#[Runtime Management Agent] installed. The agent comes bundled with Mule runtime as of version 3.6.0 and later. In case you're migrating from a Mule runtime version that is is older than that, you must do this migration manually.
...
.. Find the application on the *Aplications* tab on Runtime Manager and click *Choose File* to xref:anypoint-platform@latest::runtime-manager:managing-deployed-applications#updating-your-application[upload] the new Mule Deployable Archive (.zip file).
...
For more information about registering and unregistering Mule Servers and/or creating or disbanding a cluster of Mule Servers see xref:mule-management-console@3.8::setting-up-mmc-mule-esb-communications#[Setting Up MMC-Mule ESB Communications] and xref:mule-management-console@3.8::creating-or-disbanding-a-cluster#[Creating or Disbanding a Cluster]
...
For more information, refer to: xref:mule-management-console@3.8::upgrading-the-management-console#[Upgrading the Mule Management Console]
