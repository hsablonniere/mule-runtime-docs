...
:page-id: mule-runtime@3.8::update/updating-mule-versions
...
Before you begin, read the xref:general@master::release-notes:mule-runtime/supported-versions#[release notes] for the Mule Runtime version you wish to upgrade to (which we will call the target version, from here on), to know if you must make any necessary changes to your applications to ensure compatibility. For example, if you migrate from a version that's older than 3.6 to a version that's newer, you might want to xref:connectors:http/migrating-to-the-new-http-connector#[change all HTTP connectors] to the newer version of this connector as the old one is deprecated as of Mule Runtime 3.6.
...
====
+
. Download a new copy of your license from the Customer Portal and install it on the target version, by following the instructions described at xref:install/installing-an-enterprise-license#[Installing an Entrerprise License].
...
. xref:deploy/starting-and-stopping-mule-esb#[Run your new Mule Server] to verify your installation is working as expected.
