...
:page-id: mule-runtime@3.7::scopes/cache-scope
...
[NOTE]
The default caching strategy uses an InMemoryObject store and should only be used for testing; on actual implementations see xref:createcachingstrategy[Creating a Caching Strategy].
...
For details on configuring caching strategies, see xref:createcachingstrategy[Creating a Caching Strategy].
...
By default, Mule stores all cached responses in an InMemoryObjectStore. xref:createcachingstrategy[Creating a Caching Strategy] and define a new xref:modules/mule-object-stores#[object store] if you want to customize the way Mule stores cached responses.
...
* Read more about xref:scopes/scopes#[Scopes] in Mule
