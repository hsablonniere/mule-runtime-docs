...
:page-id: mule-runtime@3.8::scopes/message-enricher
...
You can make the source a namespace by simply populating the field with a xref:mel/mule-expression-language-mel#[Mule Expression Language MEL] expression, for example with an xpath expression that takes arguments from the Mule message.

You can find information on using a variable with any enricher listed on these pages:

* xref:non-mel-expressions/non-mel-expressions-configuration-reference#[Default enrichers] are loaded at runtime
