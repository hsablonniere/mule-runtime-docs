...
:page-id: mule-runtime@3.8::scopes/poll-reference
:keywords: anypoint studio, poll scope, polling, quartz, schedule, intervals, timing, trigger

While some connectors, such as xref:connectors:http/http-connector#[HTTP] and xref:connectors:ftp/ftp-connector#[FTP], utilize a polling process to actively retrieve messages from an external resource, most message processors in Mule are triggered when called by a previous element in a flow. If you want to arrange for a message processor to actively call a resource at regular intervals, use a Poll scope.

[TIP]
For greater flexibility, you can also configure your Poll scope to use a Cron expression, see xref:scopes/poll-schedulers#[Poll Scheduler].

*<<Complete Code Example>>*

== Prerequisites

This document assumes that you are familiar with the xref:anypoint-studio@6::about-anypoint-studio#[Visual Editor in Anypoint Studio] that you can use to create applications in Studio. Further, this document references xref:mule-configuration/global-elements#[Global Elements] and the category of message processors known as xref:scopes/scopes#[scopes].  Review xref:anypoint-studio@6::about-anypoint-studio#[Anypoint Studio Essentials] to learn more about developing with Mule's graphical user interface. Also review xref:anypoint-platform@latest::exchange:anypoint-exchange#accessing-the-exchange-s-public-content[Anypoint Exchange] for how to locate and install a connector using Exchange.
...
+
. Add more message processors to your flow to perform specific business logic after polling for data. The example below uses a xref:components/logger-component-reference#[logger] component. This logger uses the xref:mel/mule-expression-language-mel#[MEL] expression `#[payload]` to log the message payload collected by the Twitter connector every 1000 milliseconds.
+
...
|*config-ref* |string |You must create a Twitter connector as a xref:mule-configuration/global-elements#[global element] to reference here. For your Twitter connector, you need a Twitter developer account. |`config-ref=` `"Twitter"`
...
|*message* |string or xref:mel/mule-expression-language-mel#[Mule expression] |The content that logs to console. In this case, the Mule Expression `#[payload]` outputs the message payload.
...
. Run your project in Debug Mode. (Refer to the xref:anypoint-studio@6::studio-visual-debugger#[Visual Debugger] document for greater detail.)
...
Any connector which is enabled for xref:connectors:auto-paging-in-anypoint-connectors#[auto-paging] allows you to process large data sets in separate batches. This capability mitigates for memory overload, but also imposes certain conditions when used in conjunction with watermarks. The following example illustrates the recommended method for using watermarks when polling a connector that auto-pages its response.
...
* Learn how to configure a xref:scopes/poll-schedulers#[polling schedule].
...
* Learn more about xref:components/logger-component-reference#[Logger].
* Learn more about xref:connectors:anypoint-connectors#[Anypoint Connectors] and xref:connectors:auto-paging-in-anypoint-connectors#[auto-paging].
* Learn more about xref:flows/flow-processing-strategies#[flow processing strategies].
