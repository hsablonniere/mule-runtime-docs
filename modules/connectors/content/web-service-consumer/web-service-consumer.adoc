...
:page-id: mule-runtime@3.8::connectors:web-service-consumer/web-service-consumer
:keywords: anypoint studio, studio, connector, endpoint, web service, soap, wsdl
:toc: macro
:toclevels: 3
:toc-title: Contents

image:web-service-consumer-0a7fc.png[icon]
...
[WARNING]
The component formerly known as the *SOAP Connector* is now known as the *xref:mule-runtime:components/cxf-component-reference#[CXF Component]*. It should not be confused with the Web Service Consumer.

<<Complete Code Example>>

== Prerequisites

This document assumes that you are familiar with Anypoint Studio; review the chapter on xref:anypoint-studio@6::about-anypoint-studio#[Anypoint Studio Essentials] to learn more about developing with Mule's graphical user interface. This document also assumes you are familiar with xref:mule-runtime:dataweave/dataweave#[DataWeave], xref:mule-runtime:mule-configuration/global-elements#[Global Elements], and xref:anypoint-studio@6::datasense/datasense#[DataSense].
...
can use the xref:mule-runtime:modules/cxf-module-reference#[CXF module] to call the service.
...
For more information on best design-time practices with DataSense and DataWeave, consult the xref:anypoint-studio@6::datasense/datasense#[DataSense documentation].
...
To do this:
+
** Click the link *Search WSDL in Exchange* to open the xref:anypoint-platform@latest::exchange:anypoint-exchange#[Anypoint Exchange] and select a WSDL definition residing there.
...
As stated above, when used in conjunction with a DataSense-enabled Web Service Consumer, DataWeave automatically extracts the payload or header metadata that you can use to map and or transform to a different data format or structure through the xref:mule-runtime:dataweave/dataweave#[DataWeave language]. When you have a configured, DataSense-enabled Web Service Consumer in your flow, you can drop a DataWeave transformer next to it – preceding or following it – and DataWeave automatically extracts the information that DataSense collected from the WSDL about the expected format and structure of the message. 
...
Use the *xref:anypoint-studio@6::datasense/using-the-datasense-explorer#[DataSense Explorer]* feature in Studio to gain design-time insight into the state of the message payload, properties, and variables as it moves through your flow. The DataSense Explorer is mostly useful in understanding the content of a message before it encounters a Web Service Consumer, and after it emerges from the connector so as to better understand the state of the data your application is working with.
...
<mule xmlns:dw="http://www.mulesoft.org/schema/mule/ee/dw" xmlns:metadata="http://www.mulesoft.org/schema/mule/metadata" xmlns:mulexml="http://www.mulesoft.org/schema/mule/xml" xmlns:tracking="http://www.mulesoft.org/schema/mule/ee/tracking" xmlns:ws="http://www.mulesoft.org/schema/mule/ws" xmlns:http="http://www.mulesoft.org/schema/mule/http" xmlns="http://www.mulesoft.org/schema/mule/core" xmlns:doc="http://www.mulesoft.org/schema/mule/documentation"
	xmlns:spring="http://www.springframework.org/schema/beans"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xsi:schemaLocation="http://www.springframework.org/schema/beans http://www.springframework.org/schema/beans/spring-beans-current.xsd
http://www.mulesoft.org/schema/mule/core http://www.mulesoft.org/schema/mule/core/current/mule.xsd
...
http://www.mulesoft.org/schema/mule/xml http://www.mulesoft.org/schema/mule/xml/current/mule-xml.xsd">
    ...
    <ws:consumer-config name="Web_Service_Consumerweather" wsdlLocation="http://www.webservicex.com/globalweather.asmx?WSDL" service="GlobalWeather" port="GlobalWeatherSoap" serviceAddress="http://www.webservicex.com/globalweather.asmx"  doc:name="Web Service Consumer"/>
    <http:request-config name="HTTP_Request_Configuration" host="www.webservicex.com" port="80" doc:name="HTTP Request Configuration">
    ...
    <flow name="soap-wsc-proxy">
        ...
        <set-variable variableName="CountryName" value="#[message.inboundProperties.'http.query.params'.CountryName]" doc:name="Get Cities by Country"/>
...
[TIP]
--
For a full list of elements, attributes and default values, see the xref:web-service-consumer/web-service-consumer-reference#[Web Service Consumer Reference].
--
+
...
[TIP]
Learn more about passing JVM arguments to your Mule instance xref:mule-runtime:administer/passing-additional-arguments-to-the-jvm-to-control-mule#[here].
...
. Add an xref:http/http-connector#[HTTP Connector] global element in your project, configure it with the necessary security attributes
+
[source, xml, linenums]
----
...
* Learn more about how to use the xref:mule-runtime:dataweave/dataweave#[DataWeave transformer].
* Learn how to publish a REST API in Mule using xref:apikit@3.8.3::about-apikit#[APIkit].
* Access the xref:general@master::to-get-anypoint-platform#[Getting Started] to learn the basics.
