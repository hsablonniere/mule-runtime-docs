...
:page-id: mule-runtime@3.7::connectors:http/http-connector
...
* xref:http/http-listener-connector#[Listen for HTTP requests] that arrive at a certain address and provide an HTTP response to these
* xref:http/http-request-connector#[Send HTTP requests] to a certain address and receive the returned response
...
* Send xref:http/authentication-in-http-requests#[Authenticated Requests], through *Basic Authentication, Digest and OAuth*
...
* As an xref:http/http-listener-connector#[HTTP Listener Connector]
* As an xref:http/http-request-connector#[HTTP Request Connector]
...
[TIP]
To migrate a project that uses instances of the old xref:mule-runtime:transports/deprecated-http-transport-reference#[HTTP endpoint-based connector] to the new HTTP operation-based connector, see xref:http/migrating-to-the-new-http-connector#[Migrating to the New HTTP Connector]. The old HTTP endpoint-based Connector still works with Mule 3.6 Runtime, but it's deprecated and will eventually be removed.
