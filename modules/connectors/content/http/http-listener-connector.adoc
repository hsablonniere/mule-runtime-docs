...
:page-id: mule-runtime@3.8::connectors:http/http-listener-connector
...
In Studio, to instantiate the connector as an HTTP Listener Connector (and not an xref:http/http-request-connector#[HTTP Request Connector]), you must place it into the _Source_ section of a flow.
...
By default, the connector configuration has its host set to `0.0.0.0`, this address is in fact a shortcut to simultaneously listen on all active IP addresses, (including `localhost`). In order to deploy a project to xref:anypoint-platform@latest::cloudhub:cloudhub#[CloudHub], you must set the hosts of all the HTTP connectors to `0.0.0.0`. Otherwise, your application runs into conflicts with the inner workings of CloudHub.
...
When using an HTTP Listener Connector, whenever there are outbound properties included in the Mule Message that arrives to the end of the flow, each outbound property is transformed into an HTTP response header. It's easy to add properies to the Mule Message by using the xref:mule-runtime:transformers/property-transformer-reference#[Property Transformer].
...
In the example above, two headers are added to every response to a request to the app. Both headers take their values from xref:mule-runtime:mel/mule-expression-language-date-and-time-functions#[dateTime MEL expressions.]
...
. Set the variable's name to `headersOut` and its value to a map through the following xref:mule-runtime:mel/mule-expression-language-mel#[MEL expression]:  `#[['date': server.dateTime, 'expires' : server.dateTime.plusHours(1)]]`
...
See xref:mule-runtime:security/configuring-the-spring-security-manager#[Configuring the Spring Security Manager] for more details.
...
A detailed reference can be found in the xref:mule-runtime:administer/tuning-performance#connector-level[tunningg performance at the connector level] documentation.

The HTTP connector can also be configured to use a xref:http/http-connector#non-blocking-processing[non-blocking processing strategy].



== See Also

* To send HTTP requests, see  xref:http/http-request-connector#[HTTP Request Connector]
* See a xref:http/http-connector-reference#[full reference] of the available XML configurable options in this connector
*  xref:mule-runtime:api-usage/consuming-a-rest-api#[Consuming a REST API]  
*  xref:mule-runtime:api-usage/rest-api-examples#[REST API Examples]
* xref:http/authentication-in-http-requests#[Authentication in HTTP Requests] +
* To update all applications that use the old HTTP endpoint based connector, see xref:http/migrating-to-the-new-http-connector#[Migrating to the New HTTP Connector]
* Consult a reference to the deprecated predecessor of this element, xref:mule-runtime:transports/deprecated-http-transport-reference#[HTTP Transport Reference]