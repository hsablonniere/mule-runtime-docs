...
:page-id: mule-runtime@3.7::connectors:http/http-listener-connector
...
In Studio, to instance the connector as an HTTP Listener Connector (and not an xref:http/http-request-connector#[HTTP Request Connector]), you must place it into the *Source* section of a flow (ie: as the first element in the flow):
