...
:page-id: mule-runtime@3.7::connectors:http/authentication-in-http-requests
:keywords: http, authentication, security, users, connectors, anypoint, studio, esb, oauth, basic auth, digest

Use the authentication that you configure in HTTP requests when your Mule app is sending requests to a service that requires authentication, such as the Github OAuth2 server described in xref:@3.8::connectors:http/authentication-in-http-requests#oauth2-authorization-code[Mule 3.8 documentation]. In this case, your Mule app is the client. On the other hand, when you want to protect your API or app from _receiving requests_ from unauthorized access, use an API Manager policy, such as the xref:anypoint-platform@latest::api-manager:external-oauth-2.0-token-validation-policy#[OAuth 2.0 Access Token Enforcement Using External Provider] policy.
...
When this is the case, the parameters will be automatically extracted and you can easily use xref:mule-runtime:mel/mule-expression-language-mel#[MEL expressions] later in the flow to reference these values in the Mule Message that was generated from the response to the request to the token URL.
...
It’s possible to customize when Mule will perform one of these requests to obtain a new access token. This is configured through a xref:mule-runtime:mel/mule-expression-language-mel#[MEL Expression] that is evaluated against the Mule Message that is generated from the response of the HTTP Request Connector call.
