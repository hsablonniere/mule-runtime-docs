...
:page-id: mule-runtime@3.8::connectors:http/authentication-in-http-requests
:keywords: http, authentication, security, users, connectors, anypoint, studio, oauth, basic auth, digest

Use the authentication that you configure in HTTP requests when your Mule app is sending requests to a service that requires authentication, such as the Github OAuth2 server described in <<OAuth2 - Authorization Code>>. In this case, your Mule app is the client. On the other hand, when you want to protect your API or app from _receiving requests_ from unauthorized access, use an API Manager policy, such as the xref:anypoint-platform@latest::api-manager:external-oauth-2.0-token-validation-policy#[OAuth 2.0 Access Token Enforcement Using External Provider] policy.
...
+
This MEL expression xref:http/authentication-in-http-requests#extracting-parameters-from-the-token-url-response[extracts an access token].
+
...
For example, the redirectionURLis http://localhost:8082/callback in the xref:http/authentication-in-http-requests#creating-the-mule-client-app[previous example]:
...
When the response is in JSON format, the parameters are automatically extracted and you can use xref:mule-runtime:mel/mule-expression-language-mel#[MEL expressions] to reference these values in the Mule Message that was generated from the response to the request to the token URL, as shown in the previous Github example.
...
You can customize when Mule performs one of these requests to obtain a new access token using a xref:mule-runtime:mel/mule-expression-language-mel#[MEL Expression]. The expression is evaluated against the Mule Message that is generated from the response of the HTTP Request Connector call.
...
By default, the token manager uses an in-memory object store to store the credentials. You can customize the token manager object store by using the objectStore-ref attribute. xref:mule-runtime:modules/mule-object-stores#[See how to configure a custom object store].

== See Also

* HTTP Request Connector
* HTTP Listener Connector
* See a xref:http/http-connector-reference#[reference] of the available XML configurable options in this connector
* xref:http/migrating-to-the-new-http-connector#[Migrating to the New HTTP Connector]
* xref:mule-runtime:modules/object-store-module-reference#[Object Store Module Reference]