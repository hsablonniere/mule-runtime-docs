...
:page-id: mule-runtime@3.8::connectors:http/http-connector-reference
:keywords: anypoint studio, connectors, http, https, http headers, query parameters, rest, raml

Mule Utilities for HTTP Services.

This page serves as a reference for all of the supported attributes and child elements of the xref:http/http-connector#[HTTP Connector].
...
Required: no +
Default: none
|clientSocketProperties-ref |Reference to a xref:mule-runtime:transports/tcp-transport-reference#client-socket-properties[TCP Client Socket properties element]. +
Type: string +
Required: no +
Default: none
...
* Learn how to xref:mule-runtime:administer/tuning-performance#connector-level[tune performance at the connector level].
