...
:page-id: mule-runtime@3.8::connectors:http/http-request-connector
...
This connector can also implement HTTPS protocol and encrypt your communications via xref:tls/tls-configuration#[TLS], it can also implement xref:http/authentication-in-http-requests#[authentication] via basic authentication, OAuth, NTLM or digest.
...
|Enable Datasense |The *Enable DataSense* checkbox allows Studio to use xref:anypoint-studio@6::datasense/datasense#[DataSense] to expose metadata from the RAML file and interact with it on other elements in Studio.
|===
...
For this example, you must first xref:mule-runtime:transformers/variable-transformer-reference#[generate a variable] named `customMap`. If you assign that variable a value through the following MEL expression:
...
For this example you need to xref:mule-runtime:transformers/variable-transformer-reference#[create a variable] named `customMap`. If you set that variable to the MEL expression `#[{'p1':'v1', 'p2':'v2'}]`, it generates a GET request to ++http://www.example.com/test+/v1/v2+ +
 +
...
See xref:tls/tls-configuration#[TLS Configuration] for more details.
...
If you want to use a different set of HTTPS certificates, you can customize them by setting the xref:tls/tls-configuration#[TLS configuration] in the HTTP Request Connector's global configuration element. You can also create a separate TLS global element and reference it through your HTTP Connector. +
...
You can add your xref:tls/tls-configuration#[TLS] credentials as a child element of the `http:request-config` element:
...
You can also add your xref:tls/tls-configuration#[TLS] credentials in a separate construct, outside your `http:request-config` element. In that case, you must name your `tls:context` and add a `tlsContext-ref` attribute in your `http:request-config`.
...
* xref:http/http-listener-connector#[HTTP Listener Connector]
* See a xref:http/http-connector-reference#[full reference] of the available XML configurable options in this connector
* Consult a reference to the deprecated predecessor of this element, the HTTP endpoint xref:http/http-request-connector#[HTTP Transport Reference]