...
The new HTTP Connector has different syntax from the old one. Besides a different name and different attributes, the new connector must always reference a xref:mule-runtime:mule-configuration/global-elements#[global element], this is consistent with how most of the other connectors in Mule work. Multiple connectors can reference one single global element that encapsulates many of the common configuration settings.
...
* xref:http/http-request-connector#[HTTP Request Connector]
====