...
:page-id: mule-runtime@3.7::connectors:index
:keywords: studio, connectors, elements, architecture

image:connectors.png[image]

xref:anypoint-connectors#[Anypoint Connectors] receive or send messages between Mule and one or more external sources, such as files, databases, or Web services. Connectors can act as message sources by working as inbound endpoints, they can act as a message processor that performs an operation in middle of a flow, or they can fall at the end of a flow and act as the recipient of the final payload data. 
