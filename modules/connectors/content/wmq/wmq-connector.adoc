...
:page-id: mule-runtime@3.7::connectors:wmq/wmq-connector
...
You can install a connector in Anypoint Studio using the instructions in xref:anypoint-platform@latest::exchange:anypoint-exchange#[Installing Anypoint Connectors].
...
|*Enable default events tracking* |Enable default  xref:mule-runtime:mule-configuration/business-events#[business event]  tracking for this endpoint.
|===
...
See the xref:mule-runtime:transports/mule-wmq-transport-reference#[Mule WMQ Transport Reference] for details on setting the properties for a WMQ endpoint using an XML editor.
