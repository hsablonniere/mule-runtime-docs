...
:page-id: mule-runtime@3.7::connectors:using-a-connector-to-access-an-oauth-api
...
If you need to deploy to different environments (for example, production, development) where these parameters need to have different values, see xref:mule-runtime:deploy/deploying-to-multiple-environments#[Deploying to Multiple Environments].
...
* link:http://blogs.mulesoft.org/oauth-2-just-got-a-bit-easier/[Blog about using connectors with OAuth]
* xref:@3.8::connectors:http/authentication-in-http-requests#[Authentication in HTTP Requests]
* xref:anypoint-platform@latest::api-manager:external-oauth-2.0-token-validation-policy#[OAuth 2.0 Access Token Enforcement Using External Provider] policy
