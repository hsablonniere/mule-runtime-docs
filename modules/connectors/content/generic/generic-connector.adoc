...
:page-id: mule-runtime@3.8::connectors:generic/generic-connector
:keywords: anypoint studio, esb, connectors, generic connector

image:generic-connector-icon.png[generic icon]
...
Mule supports numerous protocols which can be configured as endpoints. Many, but not all of the protocols included with Mule are already configured as endpoint-based connectors available as building blocks in the connectors category of the palette. These include *FTP*, *HTTP*, and *Ajax*, for instance.
...
xref:mule-runtime:endpoints/endpoint-configuration-reference#transformers[Nest transformers] inside your generic connector from its configured endpoint(s).
